...
- Every UIKit parity case must carry a physical-device report in `oxide/benchmarks/uikit-device/latest.json` plus `oxide/benchmarks/uikit-device/latest.md`, captured on real iPhone hardware with process-scoped Metal System Trace attached only to the launched OxideHost app process. Manual per-case Power Profiler `.trace` or raw exported `.atrc` imports should be added later when direct energy is collected for that same workload.
...
- Representative scroll and animation flows must be evaluated on real 60 Hz hardware and on a ProMotion device at both 60 Hz-capped and native refresh where supported. If UIKit can use `UIScrollView` hitch metrics directly but Oxide cannot, compute the symmetric hitch ratio from frame deadlines instead of comparing unlike metrics.
- When only one ProMotion iPhone is available locally, the device report must still persist both `60hz-capped` and `native` rows for touched scroll and animation cases on that same phone, and it must explicitly note that the separate older 60 Hz hardware leg is still pending instead of implying the matrix is complete.
