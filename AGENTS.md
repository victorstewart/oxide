...
- GPU graphics work must start from the highest-performance discipline before code changes:
  - [ ] Review the relevant proven renderer designs before inventing local architecture: Skia/Graphite for retained 2D batching and GPU backends, Vello for compute-centric vector 2D, Blend2D for CPU-side analytic/JIT rasterization lessons, Rive Renderer for high-volume vector animation, Filament for compact mobile-first clustered/PBR 3D, bgfx/wgpu for backend abstraction boundaries, and Godot's split desktop/mobile renderer guidance for tile GPU tradeoffs.
  - [ ] Review the relevant Metal primary sources before touching the renderer: Metal Best Practices Guide, Metal Feature Set Tables, MTLGPUFamily capability checks, Metal Shading Language spec, Apple GPU/tile-based deferred rendering guidance, Xcode GPU Frame Capture, Metal System Trace, and current Instruments device profiling workflows.
  - [ ] Use this first-pass source stack for that review: Apple's Metal Best Practices Guide, Metal Feature Set Tables, `MTLGPUFamily`/device-inspection docs, GPU counters docs, Xcode Metal workload capture docs, Apple silicon Metal porting guide, and Metal shader performance talks; Skia docs plus Skia Graphite architecture notes; Vello architecture and Raph Levien's GPU 2D/vector-rendering posts; Filament core/material docs; bgfx internals; Godot renderer architecture and mobile renderer docs; Blend2D and Rive renderer performance notes; John Carmack's `.plan` texture-residency notes, id Software interview archives on PVS/whole-system performance, Doom/Quake/Doom 3 renderer code reviews, shadow-volume history, and Oculus Timewarp latency work.
  - [ ] Apply the Carmack/id Software discipline where it fits Oxide: optimize the whole frame pipeline rather than a single counter, prefer constant-time/coarse visibility or precomputed structure when it beats perfect dynamic culling, make data layout and texture residency predictable enough to avoid bandwidth cliffs, use front-to-back/zero-overdraw ideas only when their decision cost is lower than the overdraw they remove, and treat input/camera-to-visible latency as a first-class metric.
  - [ ] State the performance hypothesis before editing: bottleneck class, affected phase, target workload, expected counter movement, and risk to visual equivalence.
  - [ ] Prefer deleting or avoiding work over making work faster: cull earlier, coalesce draws, collapse redundant passes, shrink damaged regions, reuse cached layers only when correctness is proven, and remove stale fallback paths after hard cutover.
  - [ ] Keep the frame loop allocation-free after warmup. No per-frame pipeline creation, shader compilation, texture creation, heap churn, command queue creation, sampler/depth-state creation, string formatting, logging, or Vec growth on renderer hot paths unless the benchmark proves the exception wins.
  - [ ] Use persistent Metal objects: one device and minimal command queues per GPU, build libraries and pipeline states up front, cache sampler/depth states, allocate reusable ring buffers, and use heaps or argument buffers only after measurement shows lower CPU or residency overhead.
  - [ ] Maintain CPU/GPU overlap: use triple-buffered dynamic data, avoid CPU waits on in-flight GPU work, submit the fewest command buffers per frame that keep the GPU fed, and acquire/present drawables as late and briefly as possible.
  - [ ] Be tile-GPU native on Apple GPUs: merge compatible render encoders, keep transient attachments on chip when possible, use memoryless/private storage where appropriate, set explicit texture usage flags, use `DontCare`/`Clear`/`Load` and store/resolve actions deliberately, and never preserve or resolve an attachment whose contents are not consumed.
  - [ ] Reduce binding and draw overhead: sort/group by pipeline, texture, sampler, blend, clip, and depth state where order permits; batch same-shape primitives with instancing; bind buffers once and move offsets; atlas small textures/glyphs; use indirect command buffers only when they remove real CPU/GPU synchronization or command encoding cost.
  - [ ] Protect shader throughput: minimize overdraw, divergent branches, register pressure, precision waste, dependent texture reads, bandwidth-heavy formats, unnecessary blending, and full-resolution post effects. Use lower-resolution intermediate passes only when quality and counters justify it.
  - [ ] Preserve visual parity: Oxide and UIKit/device parity cases must draw the same scene spec, pixels, cache state, refresh mode, and interaction path. A faster path that drops quality, precision, effects, or coverage is a failed benchmark.
  - [ ] Prove every graphics refactor with A/B evidence: capture a baseline, change one coherent variable, rerun the same workload, compare p50/p95/p99/peak plus hitch/missed-frame/input latency where relevant, include GPU counters or Metal System Trace for device work, and commit refreshed baselines only when intentionally reviewed.
  - [ ] Record unresolved gaps as concrete cleanup candidates with paths and benchmark IDs. Do not call a broad graphics audit complete while any renderer, shader, bridge, scene, or device-parity family remains unaudited or unmeasured.
- Every new Oxide feature, component, animation, or renderer-facing hot path must land with a corresponding Rust perf case in `oxide/crates/perf-runner`. Run that exact case with controlled A/B evidence in the reviewed change. Promote it into the canonical workspace battery only when it adds a distinct required production path and normally replaces a lower-signal row; otherwise keep it explicit-filter-only.
...
- Every new public Oxide author-facing API surface, state controller, composition primitive, or other code path that an app author writes against must land with a corresponding `authoring` perf case in `oxide/crates/perf-runner`. Refresh the committed workspace baseline only when the canonical report shape changes; persist noncanonical touched-case evidence with the reviewed experiment instead of expanding the default battery.
...
- The committed workspace and UIKit device baselines are compact canonical signal batteries, not exhaustive case matrices. Dense count/style/backend permutations remain exact touched-case runs; do not provide or require a run-everything mode that recreates the exhaustive battery.
...
- Oxide GPU timings must not depend on Instruments hardware-counter availability. The first source of truth is in-app Metal timing collected by Oxide itself: completed `MTLCommandBuffer` `GPUStartTime`/`GPUEndTime` for command-buffer GPU duration, `MTLCounterSampleBuffer` timestamp samples attached to render passes for vertex/fragment/pass attribution when supported, and host/device stage summaries persisted into the device report. Process-scoped Metal System Trace remains the external cross-check. The `Metal GPU Counters` Instruments profile is optional hardware-counter attribution only; if Apple rejects that counter profile on a device/toolchain, keep collecting and reporting the in-app timings plus Metal System Trace GPU intervals instead of blocking basic GPU timing.
