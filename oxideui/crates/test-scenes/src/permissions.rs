...
impl Default for PermissionsScene {
    fn default() -> Self {
        ...
        let cards: Vec<PermissionCard> = domains
            .into_iter()
            .map(PermissionCard::new)
            .collect();

        // Initialize permission states
        let permission_states = cards
            .iter()
...
impl PermissionsScene {
    pub fn update(&mut self, dt_ms: u32) {
        ...
        self.permission_states = self.cards
            .iter()
            .map(|card| card.state.clone())
            .collect();
    ...
    pub fn input_pointer(&mut self, x: f32, y: f32, _dx: f32, _dy: f32, buttons: u32) {
        ...
        for (i, card) in self.cards.iter_mut().enumerate() {
            ...
            let button_rect = gfx::RectF::new(
                50.0 + col as f32 * 300.0,
                100.0 + row as f32 * 90.0,
                120.0,
                35.0,
            );
            ...
            let toggle_rect = gfx::RectF::new(
                180.0 + col as f32 * 300.0,
                105.0 + row as f32 * 90.0,
                50.0,
                25.0,
            );
    ...
    pub fn draw<U: ImageUploader>(
        ...
        if self.show_overlay && self.overlay_ui.is_visible() {
            ...
            builder.rrect(
                viewport,
                [0.0; 4],
                gfx::Color::rgba(0.0, 0.0, 0.0, 0.5),
            );
        ...
        let stats = Label {
            text: format!(
                ...
                self.total_requests,
                self.cards.iter().filter(|c| c.state.status == PermissionStatus::Authorized).count(),
                self.cards.iter().filter(|c| c.state.status == PermissionStatus::Denied).count(),
                self.cards.iter().filter(|c| c.state.status == PermissionStatus::NotDetermined).count(),
                self.cards.iter().filter(|c| c.state.status == PermissionStatus::Limited).count(),
                if self.show_overlay && self.overlay_ui.is_visible() { "Visible" } else { "Hidden" }
            ),
