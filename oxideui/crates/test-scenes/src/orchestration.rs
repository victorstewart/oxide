...
use oxideui_ui_core::{
    ...
    overlay::{OverlayStack, OverlayHandle, OverlayVisual, OverlayBehavior, OverlayPointerResult},
    DrawListBuilder, NodeId, UiSurface, NodeStyle,
};
...
pub struct OrchestrationScene {
    ...
    /// Modal content surfaces
    modal_surfaces: Vec<UiSurface>,
...
impl Default for OrchestrationScene {
    fn default() -> Self {
        ...
        let dismissable_modal_button = Button {
            text: "Dismissable Modal".into(),
            style: ButtonStyle::default(),
        };
        trigger_buttons.push((dismissable_modal_button, ButtonState::default(), "dismissable_modal".into()));

        // Create modal surfaces
        let modal_surfaces = Vec::new();
...
impl OrchestrationScene {
    ...
    pub fn input_pointer(&mut self, x: f32, y: f32, _dx: f32, _dy: f32, buttons: u32) {
        ...
        if !self.overlay_stack.is_empty() {
            ...
            match result {
                OverlayPointerResult::Dismissed { handle } => {
    ...
    fn handle_action(&mut self, action: &str) {
        match action {
            "scatter_on" => {
                let node_ids: Vec<NodeId> = self.node_groups[self.active_group]
                    .iter()
                    .map(|n| n.node_id())
                    .collect();
            ...
            "transition" => {
                ...
                let new_ids: Vec<NodeId> = self.node_groups[next_group]
                    .iter()
                    .map(|n| n.node_id())
                    .collect();
    ...
    fn show_modal(&mut self, dismissable: bool) {
        ...
        let label = if dismissable {
            "Dismissable Modal".into()
        } else {
            "Modal (ESC to close)".into()
        };
