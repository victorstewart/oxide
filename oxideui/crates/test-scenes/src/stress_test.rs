...
impl AnimatedElement {
    fn new(x: f32, y: f32) -> Self {
        Self {
            x,
            y,
            width: 20.0 + (x * 7.0) as f32 % 40.0,
            height: 20.0 + (y * 13.0) as f32 % 40.0,
            ...
            velocity_x: ((x * 17.0) as i32 % 200 - 100) as f32 * 0.01,
            velocity_y: ((y * 23.0) as i32 % 200 - 100) as f32 * 0.01,
            rotation: 0.0,
...
impl Default for StressTestScene {
    fn default() -> Self {
        ...
        Self {
            ...
            auto_increase: false,
            increase_rate: 10,
            orchestrator: ScatterOrchestrator::new(100),
            scatter_nodes: Vec::new(),
            last_update_ms: timing::now_ms(),
        }
    }
}
...
impl StressTestScene {
    ...
    fn regenerate_elements(&mut self) {
        match self.current_mode {
            ...
            StressTestMode::ManyBadges => {
                ...
                for i in 0..self.element_count {
                    ...
                    style.bounce_duration_ms = 100 + (i as u32 * 50) % 400;
        ...
        self.metrics.element_count = self.buttons.len() + self.badges.len() +
                                     self.toggles.len() + self.animated_elements.len();
    }
        ...
        for i in 0..each {
            ...
            if i < each / 2 {
                self.record_buttons.push((
                    RecordButton {
                        style: RecordButtonStyle::default(),
                    },
    ...
    pub fn draw<U: ImageUploader>(
        ...
        // Mode selector buttons
        for (i, ((button, state), mode)) in self.mode_buttons.iter_mut()
            .zip([
                StressTestMode::ManyButtons,
                StressTestMode::ManyBadges,
                StressTestMode::ManyAnimations,
        ...
        match self.current_mode {
            ...
            StressTestMode::MixedChaos => {
                ...
                for element in &self.animated_elements {
                    if draw_count >= max_per_type * 3 { break; }
                    element.draw(builder);
                    draw_count += 1;
                }
