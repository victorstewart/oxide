//! Test scenes and simple router for the OxideUI UI test app.
...
use alloc::format;
use oxideui_permissions::{PermissionManager, PermissionState, SensorBridge};
...
pub use oxideui_ui_core::camera::{CameraMetrics, CameraRecordingUiEvent};
...
pub struct Router<U: elements::ImageUploader> {
    ...
    sensors: Option<oxideui_ui_core::sensors::SensorView>,
    telemetry: Option<oxideui_ui_core::telemetry::TelemetryView>,
}
...
impl<U: elements::ImageUploader> Router<U> {
    ...
    pub fn sensors_bind(&mut self, bridge: &Arc<SensorBridge>) {
        self.sensors = Some(oxideui_ui_core::sensors::SensorView::new(Arc::clone(bridge)));
    }

    pub fn sensors_snapshot(&self) -> Option<oxideui_ui_core::sensors::SensorSnapshot> {
        self.sensors.as_ref().map(|view| view.snapshot())
    }

    pub fn telemetry_bind(&mut self, hub: &Arc<TelemetryHub>) {
        self.telemetry = Some(oxideui_ui_core::telemetry::TelemetryView::new(Arc::clone(hub)));
    }
    ...
    pub fn input_pointer(&mut self, x: f32, y: f32, dx: f32, dy: f32, buttons: u32) {
        match self.current {
            ...
            SceneKind::ElementsExtended => self.elements_extended.input_pointer(x, y, dx, dy, buttons),
            SceneKind::AnimationConfig => self.animation_config.input_pointer(x, y, dx, dy, buttons),
    ...
    pub fn input_key(&mut self, key: &oxideui_platform_api::KeyEvent) {
    ...
    pub fn draw(&mut self, viewport: gfx::RectF, device_scale: f32, b: &mut DrawListBuilder) {
        ...
        match self.current {
            ...
            SceneKind::ElementsExtended => {
                self.elements_extended.draw(viewport, device_scale, &mut self.text, &mut self.uploader, b);
                self.push_damage(rectf_to_recti(viewport));
            }
            SceneKind::AnimationConfig => {
                self.animation_config.draw(viewport, device_scale, &mut self.text, &mut self.uploader, b);
                self.push_damage(rectf_to_recti(viewport));
            }
            SceneKind::Orchestration => {
                self.orchestration.draw(viewport, device_scale, &mut self.text, &mut self.uploader, b);
                self.push_damage(rectf_to_recti(viewport));
            }
            SceneKind::Permissions => {
                self.permissions.draw(viewport, device_scale, &mut self.text, &mut self.uploader, b);
                self.push_damage(rectf_to_recti(viewport));
            }
            SceneKind::Integration => {
                self.integration.draw(viewport, device_scale, &mut self.text, &mut self.uploader, b);
                self.push_damage(rectf_to_recti(viewport));
            }
            SceneKind::StressTest => {
                self.stress_test.draw(viewport, device_scale, &mut self.text, &mut self.uploader, b);
                self.push_damage(rectf_to_recti(viewport));
            }
        }
        b.clip_pop();
...
struct FormLayout {
    tree: NodeTree,
    nodes: FormNodes,
    coordinator: AsyncLayoutCoordinator<alloc::vec::Vec<(NodeId, oxideui_ui_core::LayoutRect)>>,
...
impl Default for InputLab {
    fn default() -> Self {
        ...
        let mut submit_button = elements::Button::default();
        submit_button.text = alloc::string::String::from("Create Mission");
...
impl InputLab {
    ...
    pub fn key_event(&mut self, key: &oxideui_platform_api::KeyEvent) {
        if let Some(input) = self.focused_mut() {
            input.handle_key(key);
        }
        if matches!(key.code, oxideui_platform_api::KeyCode::Enter) {
            self.handle_submit();
        }
    }
    ...
    pub fn commit(&mut self, text: &str) {
        ...
        if let Some(input) = self.focused_mut() {
            input.handle_text_event(&oxideui_platform_api::TextEvent::Commit { text: text.into() });
        }
    }

    pub fn set_selection(&mut self, start: u32, end: u32) {
        if let Some(input) = self.focused_mut() {
            input.handle_text_event(&oxideui_platform_api::TextEvent::SelectionChanged {
                range: start..end,
            });
        }
    }
    ...
    pub fn set_composition(&mut self, start: u32, end: u32, text: &str) {
        if let Some(input) = self.focused_mut() {
            input.handle_text_event(&oxideui_platform_api::TextEvent::Composition {
                range: start..end,
                text: text.into(),
            });
        }
    ...
    pub fn set_ime_rect(&mut self, rect: gfx::RectF) {
        if let Some(input) = self.focused_mut() {
            input.handle_text_event(&oxideui_platform_api::TextEvent::IMEShown(rect));
        }
    }

    pub fn hide_ime(&mut self) {
        self.username_input.handle_text_event(&oxideui_platform_api::TextEvent::IMEHidden);
        self.password_input.handle_text_event(&oxideui_platform_api::TextEvent::IMEHidden);
    }
...
impl Default for ElementsExtended {
    fn default() -> Self {
        ...
        let mut badge_style = elements::BadgeStyle::default();
        badge_style.bounce_duration_ms = 600; // Test custom timing

        let badge = elements::Badge {
            count: 5,
            style: badge_style,
        };
        ...
        let mut record_style = elements::RecordButtonStyle::default();
        record_style.recording_timeout_ms = 5000; // 5 second timeout for testing
        record_style.press_animation_ms = 150; // Slower press animation

        let record_button = elements::RecordButton {
            style: record_style,
        };

        // Create SlidingSwitch with custom timeout
        let mut switch_style = elements::SlidingSwitchStyle::default();
        switch_style.inactive_timeout_ms = 3000; // 3 second timeout

        let sliding_switch = elements::SlidingSwitch {
            style: switch_style,
        };
...
impl ElementsExtended {
    ...
    pub fn draw<U: elements::ImageUploader>(
        ...
        title.encode(
            gfx::RectF::new(vp.x, vp.y + 10.0, vp.w, 30.0),
            ds,
            text,
            up,
            b,
        ...
        let switch_label = elements::Label {
            text: format!(
                ...
                self.sliding_switch_state.mode
            ),
            color: gfx::Color::rgba(0.3, 0.3, 0.3, 1.0),
            align: elements::Align::Left,
            wrap: false,
        ...
        info_label.encode(
            gfx::RectF::new(vp.x, vp.y + vp.h - 30.0, vp.w, 20.0),
            ds,
            text,
            up,
            b,
