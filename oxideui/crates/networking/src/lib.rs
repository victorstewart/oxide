...
impl QuicSessionManager {
                ...
                if let Some(attempt) = &mut self.handshake {
                    ...
                    if elapsed >= retry_delay {
                        if attempt.attempts < self.config.max_retries {
                            ...
                            let next_delay = (attempt.current_delay_ms as f32 * self.config.backoff_multiplier)
                                .min(self.config.max_retry_ms as f32) as u32;
    ...
    fn send_handshake(&mut self, now_ms: u64, kind: PacketKind) {
        self.enqueue_packet(kind.clone(), vec![0x01, kind as u8], now_ms);
        let (attempts, delay_ms) = match kind {
            ...
            PacketKind::HandshakeRetry => {
                ...
                let delay = existing.map(|h| h.current_delay_ms).unwrap_or(self.config.initial_retry_ms);
                (attempts, delay)
            }
            PacketKind::TimeSyncProbe => (0, 0),
        };
        self.handshake = Some(HandshakeAttempt {
            attempts,
            last_sent_ms: now_ms,
