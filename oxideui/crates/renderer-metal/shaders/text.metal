...
struct TextVSIn { float2 pos [[attribute(0)]]; float2 uv [[attribute(1)]]; ushort4 rgba [[attribute(2)]]; };
...
vertex TextVSOut v_text(TextVSIn in [[stage_in]], constant TextVPSize& vp [[buffer(1)]]) {
    ...
    clip.y = (in.pos.y / max(vp.size.y, 1.0)) * 2.0 - 1.0;
    o.position = float4(clip, 0.0, 1.0);
    o.uv = in.uv;
    return o;
}

fragment float4 f_text(TextVSOut in [[stage_in]], texture2d<float> atlas [[texture(0)]], sampler s [[sampler(0)]], constant TextUniform& uni [[buffer(1)]]) {
...
fragment float4 f_text_sdf(TextVSOut in [[stage_in]], texture2d<float> atlas [[texture(0)]], sampler s [[sampler(0)]], constant TextUniform& uni [[buffer(1)]])
{
