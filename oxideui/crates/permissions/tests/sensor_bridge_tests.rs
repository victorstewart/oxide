...
use oxideui_platform_api::{
    ...
    PermissionStatus, PushNotification, PushToken,
};
use std::sync::{
...
fn permit(bridge: &SensorBridge, domain: PermissionDomain, status: PermissionStatus) {
    bridge.update_permission(oxideui_permissions::PermissionState::new(domain, status, 0));
}

#[test]
fn location_history_prunes_by_length_and_age() {
    let (now, clock) = test_clock_pair();
    let config = oxideui_permissions::sensors::SensorBridgeConfig {
...
fn motion_history_clears_on_revocation() {
    let (now, clock) = test_clock_pair();
    let mut config = oxideui_permissions::sensors::SensorBridgeConfig::default();
    config.motion_history_max = 4;
    let bridge = SensorBridge::new_with_config(clock, config);
...
fn bluetooth_prunes_by_age_limit() {
    let (now, clock) = test_clock_pair();
    let mut config = oxideui_permissions::sensors::SensorBridgeConfig::default();
    config.bluetooth_max_age_ms = 100;
    let bridge = SensorBridge::new_with_config(clock, config);
    permit(&bridge, PermissionDomain::Bluetooth, PermissionStatus::Authorized);

    let info = oxideui_platform_api::PeripheralInfo {
        id: 1,
        name: Some("dev".into()),
        rssi_dbm: -40,
        advertisement: oxideui_platform_api::AdvertisementData {
            services: Vec::new(),
...
fn push_notifications_tracked_when_authorized() {
    ...
    bridge.set_push_token(Some(PushToken {
        provider: oxideui_platform_api::PushProvider::Apns,
        value: "abc".into(),
    }));
    let mut notification =
        PushNotification { user_info: Default::default(), badge: Some(1), sound: None };
