use clap::Parser;
use oxideui_platform_api as plat;
...
#[derive(Parser, Debug)]
#[command(name = "oxideui-snapshot-runner")]
struct Args {
...
fn load_png_rgba(path: &Path) -> anyhow::Result<(u32, u32, Vec<u8>)> {
    ...
    let mut dec = png::Decoder::new(&buf[..]);
...
fn render_component(
    ...
    match comp.to_ascii_lowercase().as_str() {
        ...
        "scene_controls" | "scene_text" | "scene_zoom" | "scene_collection" => {
            let ptr: *mut metal::MetalRenderer = renderer;
            let mut router = ui::scenes::Router::new(Uploader { r: ptr });
            ...
            router.update(oxideui_timing::now_ms(), dt);
        ...
        "animtimeline" => {
            let mut at = ui::scenes::AnimTimeline::default();
...
fn main() -> anyhow::Result<()> {
    ...
    write_png_rgba(&args.out, w, h, &bgra)?;
