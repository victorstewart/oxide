...
use oxideui_timing::testing;
use oxideui_timing::{self, advance_timers, anim, now_ms, now_ns, schedule_after};
...
fn reduce_motion_zeroes_duration() {
    let _guard = TEST_LOCK.lock().unwrap();
    testing::reset();
    oxideui_timing::set_reduce_motion(true);
    ...
    oxideui_timing::set_reduce_motion(false);
    anim::cancel(id);
    assert_eq!(testing::active_anims(), 0);
}
