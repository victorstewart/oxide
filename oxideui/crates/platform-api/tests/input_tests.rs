...
use oxideui_renderer_api::{Insets, RectF};
...
fn touch_event_equality_and_device() {
    let t0 = TouchEvent {
        id: oxideui_platform_api::TouchId(7),
...
fn anim_desc_clone_roundtrip() {
    let desc = AnimDesc {
        ...
        curve: AnimCurve::Ease {
            ease: oxideui_platform_api::Ease { kind: oxideui_platform_api::EaseKind::CubicInOut },
        },
        duration_ms: 200,
        delay_ms: 10,
        repeat: oxideui_platform_api::Repeat::Count(2),
    };
...
fn text_event_variants() {
    let commit = AppEvent::Text(oxideui_platform_api::TextEvent::Commit { text: "hello".into() });
    match commit {
        AppEvent::Text(oxideui_platform_api::TextEvent::Commit { text }) => {
            assert_eq!(text, "hello")
        }
        _ => panic!("unexpected variant"),
    }

    let ime = oxideui_platform_api::TextEvent::IMEShown(RectF::new(0.0, 0.0, 100.0, 50.0));
    match ime {
        oxideui_platform_api::TextEvent::IMEShown(rect) => assert_eq!(rect.w, 100.0),
