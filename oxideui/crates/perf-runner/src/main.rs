...
fn run_scene(
    r: &mut metal::MetalRenderer,
    router: &mut ui::scenes::Router<Uploader>,
    frames: usize,
    vp: api::RectF,
    scale: f32,
) {
...
fn main() {
    ...
    std::env::set_var("OXIDEUI_ENABLE_DAMAGE", "1");
    ...
    let mut router = ui::scenes::Router::new(uploader);
    ...
    let use_list = std::env::var("OXIDEUI_SWEEP_USE").ok();
    let pf_list = std::env::var("OXIDEUI_SWEEP_PREFILTER").ok();
    ...
    if let (Some(ul), Some(pl)) = (use_list, pf_list) {
        ...
        for u in use_vals.iter().copied() {
            for p in pf_vals.iter().copied() {
                // Recreate renderer to pick up env
                drop(boxed);
                std::env::set_var("OXIDEUI_DAMAGE_USE_THRESH", format!("{}", u));
                std::env::set_var("OXIDEUI_DAMAGE_PREFILTER_THRESH", format!("{}", p));
                ...
                let mut router2 = ui::scenes::Router::new(uploader2);
