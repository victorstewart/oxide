//! OxideUI UI Core (minimal CPU utilities)
#![forbid(unsafe_code)]
...
pub mod prelude {
    pub use oxideui_platform_api as platform_api;
    pub use oxideui_renderer_api as renderer_api;
    pub use oxideui_utils as utils;
}
...
///
/// Rules:
/// - GlyphRun: same atlas + color, adjacent in the list, and both vertex/index spans are
///   contiguous in the backing arrays -> extend the first spans and remove the second.
    ...
    while i + 1 < list.items.len() {
        let can_merge = match (&list.items[i], &list.items[i + 1]) {
            (C::GlyphRun { run: a }, C::GlyphRun { run: b }) => {
                a.atlas == b.atlas
                    && a.color == b.color
                    && a.vb.offset + a.vb.len == b.vb.offset
                    && a.ib.offset + a.ib.len == b.ib.offset
            }
            (C::Solid { vb: av, ib: ai, color: ac }, C::Solid { vb: bv, ib: bi, color: bc }) => {
                ac == bc && av.offset + av.len == bv.offset && ai.offset + ai.len == bi.offset
            }
            _ => false,
        };
        if can_merge {
