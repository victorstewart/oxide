...
impl Label {
    pub fn encode<U: ImageUploader>(
        ...
        let Some(font) = txt.fonts.font(self.font_id) else { return };
        // Ensure a GPU handle exists to reference in glyph runs
        let handle = if let Some(h) = txt.atlas_handle {
            h
        } else {
            let (d, w, h) = txt.atlas.image();
            let hdl = up.create_a8(w, h, d, w as usize);
            txt.atlas_handle = Some(hdl);
            hdl
        };
        ...
        // Upload full atlas after baking new glyphs
        let (data, w, h) = txt.atlas.image();
        up.update_a8(handle, 0, 0, w, h, data, w as usize);
    }
}
...
impl BadgeState {
    ...
    fn current_scale(&self) -> f32 {
        ...
        let scale = if t < 0.5 {
            ...
            let local = t * 2.0;
            self.bounce_from_scale + (self.bounce_to_scale - self.bounce_from_scale) * ease_out_back(local)
        } else {
...
impl RecordButton {
    pub fn encode(
        ...
        if state.mode == RecordButtonMode::Recording {
            ...
            let progress_bar_rect = gfx::RectF::new(
                rect.x,
                rect.y + rect.h + 8.0,
                rect.w,
                4.0,
...
impl ShiftingTextInput {
    ...
    pub fn encode<U: ImageUploader>(
        ...
        let adjusted_rect = gfx::RectF::new(
            rect.x + shake_x,
            rect.y,
            rect.w,
            rect.h,
        ...
        let bg = match state.validation {
            ...
            _ => if state.focused { style.background_focus } else { style.background },
        };
