...
- Rust (see `oxideui/rust-toolchain.toml` for pinned version).
...
- `oxideui-renderer-metal` features:
...
```
cargo test -p oxideui-renderer-metal --features snapshot-tests
```
...
```
cargo test -p oxideui-input
cargo test -p oxideui-ui-core
```
