...
cargo +"${TOOLCHAIN}" build --package oxideui-host-ios --release --target aarch64-apple-ios-sim
cargo +"${TOOLCHAIN}" build --package oxideui-host-ios --release --target aarch64-apple-ios
popd >/dev/null
