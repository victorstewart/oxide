...
Minimal AppKit application that hosts a Metal-backed view for OxideUI UI testing.

- Staticlib: `oxideui-host-macos` exposes `rust_entry()` that starts the app.
- Runner: `oxideui-macos-app` binary calls `rust_entry()`.
- View: `MetalView` uses `CAMetalLayer` and configures drawable size on layout.

Build and run:

```
cargo run -p oxideui-macos-app
```
...
```
cargo bundle --bin oxideui-macos-app --release
```

This produces `target/release/bundle/osx/oxideui-macos-app.app` containing
...
```rust
// macOS only
if let Some(bytes) = oxideui_host_macos::macos_resource_get("fonts/Inter-Regular.ttf") {
    // use bytes
}
```
