...
- (void)applicationDidFinishLaunching:(NSNotification *)notification
{
    ...
    self.window = [[NSWindow alloc] initWithContentRect:frame
                                              styleMask:(NSWindowStyleMaskTitled | NSWindowStyleMaskClosable | NSWindowStyleMaskResizable)
                                                backing:NSBackingStoreBuffered
                                                  defer:NO];
    [self.window setTitle:@"OxideUI macOS Test Host"];
    ...
    NSLog(@"[OxideUI] App launched; window ready");
...
void macos_set_idle_timer_disabled(uint8_t disabled)
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (disabled) {
            if (!activity) {
                activity = [[NSProcessInfo processInfo] beginActivityWithOptions:NSActivityIdleSystemSleepDisabled reason:@"OxideUI test app running"];
            }
        } else {
...
- (void)startDisplayLink
{
    ...
    CVDisplayLinkSetOutputCallback(gDisplayLink, &DisplayLinkCallback, NULL);
...
- (void)stopDisplayLink
{
    ...
    CVDisplayLinkRelease(gDisplayLink);
    gDisplayLink = NULL;
}
...
{
    if (!gDisplayLink) return;
    CVTime t = CVDisplayLinkGetNominalOutputVideoRefreshPeriod(gDisplayLink);
    if (t.timeValue != 0) {
                                    ...
                                    void *displayLinkContext)
{
    ...
    if (gDropEvery > 1) {
        ...
        if ((idx % gDropEvery) != 0) { return kCVReturnSuccess; }
    }
    // Dispatch to main
    dispatch_async(dispatch_get_main_queue(), ^{
        // Update FPS overlay once per second
        gFpsCount += 1;
