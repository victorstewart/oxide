use oxideui_host_ios::{
    oxideui_host_emit_ime_hidden, oxideui_host_emit_ime_shown, oxideui_host_emit_push_notify,
    oxideui_host_emit_push_token, oxideui_host_emit_text_commit,
    oxideui_host_emit_text_composition, oxideui_host_emit_text_selection,
    oxideui_host_emit_window_resized, oxideui_host_set_ime_callbacks,
    oxideui_host_set_push_notify_callback, oxideui_host_set_push_token_callback,
    oxideui_host_set_text_commit_callback, oxideui_host_set_text_composition_callback,
    oxideui_host_set_text_selection_callback, oxideui_host_set_window_resized_callback,
};
use std::sync::{
...
static WINDOW_ARGS: Mutex<Option<(f32, f32, f32, [f32; 4])>> = Mutex::new(None);
...
fn window_resize_callback_invoked() {
    oxideui_host_set_window_resized_callback(Some(window_cb));
    oxideui_host_emit_window_resized(100.0, 200.0, 2.0, 1.0, 2.0, 3.0, 4.0);
...
fn text_callbacks_forward_payload() {
    ...
    oxideui_host_emit_text_commit(b"hello".as_ptr(), 5);
    assert_eq!(TEXT_COMMIT.lock().unwrap().take(), Some("hello".into()));
    oxideui_host_set_text_commit_callback(None);

    oxideui_host_set_text_composition_callback(Some(text_compose_cb));
    oxideui_host_emit_text_composition(1, 4, b"abcd".as_ptr(), 4);
    assert_eq!(TEXT_COMPOSE.lock().unwrap().take(), Some((1, 4, "abcd".into())));
    oxideui_host_set_text_composition_callback(None);

    oxideui_host_set_text_selection_callback(Some(text_select_cb));
    oxideui_host_emit_text_selection(3, 7);
...
fn push_callbacks_capture_data() {
    oxideui_host_set_push_token_callback(Some(push_token_cb));
    let token = b"abcdef";
    oxideui_host_emit_push_token(9, token.as_ptr(), token.len());
    assert_eq!(PUSH_TOKEN.lock().unwrap().take(), Some((9, "abcdef".into())));
    oxideui_host_set_push_token_callback(None);

    oxideui_host_set_push_notify_callback(Some(push_notify_cb));
    oxideui_host_emit_push_notify(b"{\"aps\":1}".as_ptr(), 9);
    assert_eq!(PUSH_NOTIFY.lock().unwrap().take(), Some("{\"aps\":1}".into()));
    oxideui_host_set_push_notify_callback(None);
}

#[test]
fn ime_callbacks_record_events() {
    oxideui_host_set_ime_callbacks(Some(ime_shown_cb), Some(ime_hidden_cb));
    oxideui_host_emit_ime_shown(10.0, 20.0, 30.0, 40.0);
