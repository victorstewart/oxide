//! OxideUI iOS host static library
//!
...
use oxideui_networking::ReachabilityManager;
#[cfg(target_os = "ios")]
...
use oxideui_permissions::{
    PermissionManager, PermissionState, PermissionSubscription, SensorBridge,
};
#[cfg(target_os = "ios")]
use oxideui_platform_api::{
    BluetoothEvent, LocationOptions, PermissionDomain, PermissionStatus, Permissions,
    PlatformError, PushProvider, PushToken,
};
use oxideui_renderer_api as gfx_api;
use oxideui_renderer_api::Renderer;
use oxideui_renderer_metal as metal;
...
#[cfg(target_os = "ios")]
use oxideui_platform_ios::{
    IosBluetooth, IosCameraManager, IosLocation, IosMotion, IosPermissions, IosPushManager,
    IosReachability,
};
...
extern "C" {
    ...
    fn oxideui_host_start() -> ::core::ffi::c_int;
...
struct SensorRuntime {
    ...
    binding: oxideui_permissions::sensors::SensorPermissionBinding,
...
impl SensorRuntime {
    ...
    fn install_push_token_bridge(&self) {
        ...
        oxideui_host_set_push_token_callback(Some(sensor_push_token_cb));
    }
...
impl NetworkRuntime {
    ...
    fn metrics(&self) -> oxideui_networking::QuicSessionMetrics {
...
pub extern "C" fn rust_entry() -> ::libc::c_int {
    ...
    unsafe {
        ...
        {
            oxideui_platform_ios::init_tokio_spawn();
        }
        ios_log("rust_entry: calling oxideui_host_start");
        let rc = oxideui_host_start() as ::libc::c_int;
        ios_log(&format!("rust_entry: oxideui_host_start returned {}", rc));
        return rc;
    }
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_window_resized_callback(cb: Option<WindowResizedCb>) {
    let slot = WINDOW_CB.get_or_init(|| std::sync::Mutex::new(None));
    *slot.lock().expect("window cb mutex poisoned") = cb;
}

#[no_mangle]
pub extern "C" fn oxideui_host_emit_window_resized(
    w: f32,
    h: f32,
    scale: f32,
    safe_l: f32,
    ...
    if let Some(cb) = WINDOW_CB.get().and_then(|m| *m.lock().expect("window cb mutex poisoned")) {
        ...
        eprintln!(
            "[OxideUI] window resized: {:.1}x{:.1} scale={:.2} safe=({:.1},{:.1},{:.1},{:.1})",
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_text_commit_callback(cb: Option<TextCommitCb>) {
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_perm_callback(cb: Option<PermCb>) {
...
extern "C" fn sensor_push_token_cb(provider: u32, ptr: *const u8, len: usize) {
    unsafe {
        oxideui_platform_ios::oxideui_push_token_trampoline(provider, ptr, len);
    }
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_push_token_callback(cb: Option<PushTokenCb>) {
    let slot = PUSH_TOKEN_CB.get_or_init(|| std::sync::Mutex::new(None));
    *slot.lock().expect("push token cb mutex") = cb;
}

#[no_mangle]
pub extern "C" fn oxideui_host_set_push_notify_callback(cb: Option<PushNotifyCb>) {
    let slot = PUSH_NOTIFY_CB.get_or_init(|| std::sync::Mutex::new(None));
    *slot.lock().expect("push notify cb mutex") = cb;
}

#[no_mangle]
pub extern "C" fn oxideui_host_emit_push_token(provider: u32, ptr: *const u8, len: usize) {
    if let Some(cb) = PUSH_TOKEN_CB.get().and_then(|m| *m.lock().unwrap()) {
        ...
        eprintln!("[OxideUI] push token (prov={}): {}", provider, s);
    }
}

#[no_mangle]
pub extern "C" fn oxideui_host_emit_push_notify(ptr: *const u8, len: usize) {
    if let Some(cb) = PUSH_NOTIFY_CB.get().and_then(|m| *m.lock().unwrap()) {
        cb(ptr, len);
    } else if let Ok(s) = std::str::from_utf8(unsafe { std::slice::from_raw_parts(ptr, len) }) {
        eprintln!("[OxideUI] push notify: {}", s);
    }
}

#[no_mangle]
pub extern "C" fn oxideui_host_emit_perm(domain: u32, status: u32) {
    if let Some(cb) = PERM_CB.get().and_then(|m| *m.lock().unwrap()) {
        cb(domain, status);
    } else {
        eprintln!("[OxideUI] perm domain={} status={}", domain, status);
    }
}
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_touch_callback(cb: Option<TouchCb>) {
...
#[no_mangle]
pub extern "C" fn oxideui_host_ble_set_state_cb(cb: Option<BleStateCb>) {
    let s = BLE_STATE_CB.get_or_init(|| std::sync::Mutex::new(None));
    *s.lock().unwrap() = cb;
}
#[no_mangle]
pub extern "C" fn oxideui_host_ble_set_discovered_cb(cb: Option<BleDiscoveredCb>) {
    let s = BLE_DISCOVERED_CB.get_or_init(|| std::sync::Mutex::new(None));
    *s.lock().unwrap() = cb;
}
#[no_mangle]
pub extern "C" fn oxideui_host_ble_set_connected_cb(cb: Option<BleConnCb>) {
    let s = BLE_CONN_CB.get_or_init(|| std::sync::Mutex::new(None));
    *s.lock().unwrap() = cb;
}
#[no_mangle]
pub extern "C" fn oxideui_host_ble_set_disconnected_cb(cb: Option<BleDiscCb>) {
    let s = BLE_DISC_CB.get_or_init(|| std::sync::Mutex::new(None));
    *s.lock().unwrap() = cb;
}
#[no_mangle]
pub extern "C" fn oxideui_host_ble_set_notify_cb(cb: Option<BleNotifyCb>) {
    let s = BLE_NOTIFY_CB.get_or_init(|| std::sync::Mutex::new(None));
    *s.lock().unwrap() = cb;
}

#[no_mangle]
pub extern "C" fn oxideui_host_ble_emit_state(powered_on: u8) {
    if let Some(cb) = BLE_STATE_CB.get().and_then(|m| *m.lock().unwrap()) {
        cb(powered_on);
    }
}
#[no_mangle]
pub extern "C" fn oxideui_host_ble_emit_discovered(info: *const OxideBleScanInfo) {
    if let Some(cb) = BLE_DISCOVERED_CB.get().and_then(|m| *m.lock().unwrap()) {
        cb(info);
    }
}
#[no_mangle]
pub extern "C" fn oxideui_host_ble_emit_connected(id: *const u8) {
    if let Some(cb) = BLE_CONN_CB.get().and_then(|m| *m.lock().unwrap()) {
        cb(id);
    }
}
#[no_mangle]
pub extern "C" fn oxideui_host_ble_emit_disconnected(id: *const u8) {
    if let Some(cb) = BLE_DISC_CB.get().and_then(|m| *m.lock().unwrap()) {
        cb(id);
    }
}
#[no_mangle]
pub extern "C" fn oxideui_host_ble_emit_notified(
    id: *const u8,
    svc: *const u8,
    chr: *const u8,
    data: *const u8,
...
pub extern "C" fn oxideui_host_emit_touch(
    ...
    if let Some(cb) = TOUCH_CB.get().and_then(|m| *m.lock().unwrap()) {
        ...
        eprintln!("[OxideUI] touch id={} phase={} x={} y={}", id, phase, x, y);
    }
}

#[no_mangle]
pub extern "C" fn oxideui_host_emit_pointer(
    x: f32,
    y: f32,
    dx: f32,
    dy: f32,
    ...
    if let Some(cb) = POINTER_CB.get().and_then(|m| *m.lock().unwrap()) {
        ...
        eprintln!("[OxideUI] pointer x={} y={} dx={} dy={}", x, y, dx, dy);
    }
}

#[no_mangle]
pub extern "C" fn oxideui_host_emit_key(
    code: u32,
    chars_ptr: *const u8,
    chars_len: usize,
    repeat: u8,
    ...
    if let Some(cb) = KEY_CB.get().and_then(|m| *m.lock().unwrap()) {
        ...
        eprintln!("[OxideUI] key code={} chars='{}' repeat={} mods={}", code, s, repeat, modifiers);
    }
}

#[no_mangle]
pub extern "C" fn oxideui_host_set_text_composition_callback(cb: Option<TextCompositionCb>) {
    let slot = TEXT_COMPOSE_CB.get_or_init(|| std::sync::Mutex::new(None));
    *slot.lock().expect("compose cb mutex") = cb;
}

#[no_mangle]
pub extern "C" fn oxideui_host_set_text_selection_callback(cb: Option<TextSelectionCb>) {
    let slot = TEXT_SELECT_CB.get_or_init(|| std::sync::Mutex::new(None));
    *slot.lock().expect("select cb mutex") = cb;
}

#[no_mangle]
pub extern "C" fn oxideui_host_set_ime_callbacks(
...
#[no_mangle]
pub extern "C" fn oxideui_host_emit_text_commit(ptr: *const u8, len: usize) {
    if let Some(cb) = TEXT_COMMIT_CB.get().and_then(|m| *m.lock().unwrap()) {
        ...
        if let Ok(s) = std::str::from_utf8(unsafe { std::slice::from_raw_parts(ptr, len) }) {
            eprintln!("[OxideUI] text commit: {}", s);
        }
    }
}

#[no_mangle]
pub extern "C" fn oxideui_host_emit_text_composition(
    start: u32,
    end: u32,
    ptr: *const u8,
    len: usize,
    ...
    if let Some(cb) = TEXT_COMPOSE_CB.get().and_then(|m| *m.lock().unwrap()) {
        ...
        if let Ok(s) = std::str::from_utf8(unsafe { std::slice::from_raw_parts(ptr, len) }) {
            eprintln!("[OxideUI] text composition: [{}..{}] {}", start, end, s);
        }
    }
}

#[no_mangle]
pub extern "C" fn oxideui_host_emit_text_selection(start: u32, end: u32) {
    if let Some(cb) = TEXT_SELECT_CB.get().and_then(|m| *m.lock().unwrap()) {
        cb(start, end);
    } else {
        eprintln!("[OxideUI] selection: [{}..{}]", start, end);
    }
}

#[no_mangle]
pub extern "C" fn oxideui_host_emit_ime_shown(x: f32, y: f32, w: f32, h: f32) {
    if let Some(cb) = IME_SHOWN_CB.get().and_then(|m| *m.lock().unwrap()) {
        cb(x, y, w, h);
    } else {
        eprintln!("[OxideUI] IME shown at {},{} size {}x{}", x, y, w, h);
    }
}

#[no_mangle]
pub extern "C" fn oxideui_host_emit_ime_hidden() {
    if let Some(cb) = IME_HIDDEN_CB.get().and_then(|m| *m.lock().unwrap()) {
        cb();
    } else {
        eprintln!("[OxideUI] IME hidden");
    }
}
...
extern "C" {
    fn oxideui_host_resource_read(
        name: *const ::libc::c_char,
        out_ptr: *mut *mut u8,
        out_len: *mut usize,
    ) -> ::libc::c_int;
    fn oxideui_host_string_free(p: *mut u8);
    fn oxideui_host_release_drawable(drawable: *mut ::libc::c_void);
}
...
struct AppState {
    ...
    network_metrics: Option<oxideui_networking::QuicSessionMetrics>,
...
fn with_app_mut<R>(f: impl FnOnce(&mut AppState) -> R) -> Option<R> {
    APP_STATE.get().and_then(|mtx| mtx.lock().ok()).map(|mut guard| f(&mut guard))
}
...
fn resource_read(name: &str) -> Option<Vec<u8>> {
    ...
    let mut len: usize = 0;
    let ok = unsafe { oxideui_host_resource_read(c_name.as_ptr(), &mut ptr, &mut len) };
...
#[no_mangle]
pub extern "C" fn oxideui_host_app_init(w: u32, h: u32, scale: f32) -> ::libc::c_int {
    #[cfg(target_os = "ios")]
    ...
    let mut renderer = match metal::MetalRenderer::new_default() {
        Ok(r) => r,
        Err(err) => {
            eprintln!("[OxideUI] MetalRenderer::new_default() failed: {err:?}");
            return -1;
        }
    };
    ...
    #[cfg(target_os = "ios")]
    {
        let manager: Arc<dyn api::CameraManager + Send + Sync> = Arc::new(IosCameraManager);
    ...
    oxideui_host_set_window_resized_callback(Some(window_resized_cb));
    oxideui_host_set_touch_callback(Some(touch_cb));
    oxideui_host_set_pointer_callback(Some(pointer_cb));
    oxideui_host_set_key_callback(Some(key_cb));
    oxideui_host_set_text_commit_callback(Some(text_commit_cb));
    oxideui_host_set_text_composition_callback(Some(text_composition_cb));
    oxideui_host_set_text_selection_callback(Some(text_selection_cb));
    oxideui_host_set_ime_callbacks(Some(ime_shown_cb), Some(ime_hidden_cb));
    app.inited = true;
    #[cfg(target_os = "ios")]
...
#[no_mangle]
pub extern "C" fn oxideui_host_app_frame(w: u32, h: u32, scale: f32) -> ::libc::c_int {
    let mut app = app_state().lock().expect("app_state mutex");
    if !app.inited {
        return -1;
    }
    ...
    let perf_stats = {
        ...
        let token = renderer.begin_frame(&gfx_api::FrameTarget, Some(&damage));
        {
            ...
            oxideui_ui_core::coalesce_adjacent_draws(dl);
        }
...
#[no_mangle]
pub extern "C" fn oxideui_host_app_did_enter_background() {
    with_app_mut(|app| {
        if let Some(ops) = app.telemetry_ops.as_ref() {
...
#[no_mangle]
pub extern "C" fn oxideui_host_app_will_enter_foreground() {
    with_app_mut(|app| {
        if let Some(ops) = app.telemetry_ops.as_ref() {
            ops.handle_foreground(timing::now_ms());
        }
...
#[no_mangle]
pub extern "C" fn oxideui_host_app_will_terminate() {
    with_app_mut(|app| {
        if let Some(ops) = app.telemetry_ops.as_ref() {
...
#[no_mangle]
pub extern "C" fn oxideui_host_on_memory_warning() {
    with_app_mut(|app| {
    ...
    .unwrap_or_else(|| {
        eprintln!("[OxideUI] memory warning received before app init");
    });
}

#[no_mangle]
pub extern "C" fn oxideui_host_app_present(
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_camera_options(
    blur: u8,
    sigma: f32,
    grayscale: u8,
    animate: u8,
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_camera_running(on: u8) -> ::libc::c_int {
    let mut app = app_state().lock().expect("app_state mutex");
    let want_on = on != 0;
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_anim_play(play: u8) -> ::libc::c_int {
    let mut app = app_state().lock().expect("app_state mutex");
    let router = match app.router.as_mut() {
        Some(r) => r,
        None => return -1,
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_anim_progress(progress: f32) -> ::libc::c_int {
    let mut app = app_state().lock().expect("app_state mutex");
    let router = match app.router.as_mut() {
        Some(r) => r,
        None => return -1,
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_damage_options(
    enabled: u8,
    use_thresh: f32,
    prefilter: f32,
) -> ::libc::c_int {
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_nine_slice(slice_px: f32, alpha: f32) -> ::libc::c_int {
    let mut app = app_state().lock().expect("app_state mutex");
    let router = match app.router.as_mut() {
        Some(r) => r,
        None => return -1,
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_sdf_font(font_px: f32) -> ::libc::c_int {
    let mut app = app_state().lock().expect("app_state mutex");
    let router = match app.router.as_mut() {
        Some(r) => r,
        None => return -1,
...
#[no_mangle]
pub extern "C" fn oxideui_host_take_snapshot() -> ::libc::c_int {
    let mut app = app_state().lock().expect("app_state mutex");
    ...
    let outcome: Result<(std::path::PathBuf, u32, u32), String> = (|| {
        ...
        let filename = format!("oxideui-snapshot-{}-{}x{}.png", timing::now_ms(), w, h);
        path.push(filename);
...
#[no_mangle]
pub extern "C" fn oxideui_host_get_snapshot_status(
...
#[no_mangle]
pub extern "C" fn oxideui_host_input_log(ptr: *const u8, len: usize) {
    if ptr.is_null() || len == 0 {
        return;
    }
...
#[no_mangle]
pub extern "C" fn oxideui_host_app_shutdown() {
    if let Some(state) = APP_STATE.get() {
        let mut app = state.lock().expect("app_state mutex");
        oxideui_host_set_window_resized_callback(None);
        oxideui_host_set_touch_callback(None);
        oxideui_host_set_pointer_callback(None);
        oxideui_host_set_key_callback(None);
        oxideui_host_set_text_commit_callback(None);
        oxideui_host_set_text_composition_callback(None);
        oxideui_host_set_text_selection_callback(None);
        oxideui_host_set_ime_callbacks(None, None);
        if let Some(router) = app.router.as_mut() {
...
#[no_mangle]
pub extern "C" fn oxideui_host_app_stats(out: *mut OxideHostStats) -> ::libc::c_int {
    if out.is_null() {
        return -1;
    }
    if let Some(state) = APP_STATE.get() {
...
#[no_mangle]
pub extern "C" fn oxideui_host_scene_count() -> u32 {
    test_scenes::Router::<MtlUploader>::scene_names().len() as u32
}

#[no_mangle]
pub extern "C" fn oxideui_host_scene_name(index: u32, out_ptr: *mut u8, out_len: usize) -> u32 {
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_scene(index: u32) -> ::libc::c_int {
    let count = oxideui_host_scene_count();
    if index >= count {
        return -1;
    }
    with_app_mut(|app| {
...
#[no_mangle]
pub extern "C" fn oxideui_host_current_scene() -> u32 {
    APP_STATE
        .get()
...
#[no_mangle]
pub extern "C" fn oxideui_host_set_overlay_visible(on: u8) -> ::libc::c_int {
    let desired = on != 0;
    with_app_mut(|app| {
        if let Some(router) = app.router.as_mut() {
            if app.overlay_visible != desired {
                router.toggle_overlay();
...
#[no_mangle]
pub extern "C" fn oxideui_host_is_overlay_visible() -> u8 {
    APP_STATE
        .get()
        .and_then(|state| state.lock().ok())
        .map(|app| if app.overlay_visible { 1 } else { 0 })
        .unwrap_or(0)
}

#[no_mangle]
pub extern "C" fn oxideui_host_set_reduce_motion(on: u8) -> ::libc::c_int {
    let desired = on != 0;
    with_app_mut(|app| {
        if let Some(router) = app.router.as_mut() {
            if app.reduce_motion_on != desired {
...
#[no_mangle]
pub extern "C" fn oxideui_host_is_reduce_motion() -> u8 {
    APP_STATE
        .get()
        .and_then(|state| state.lock().ok())
        .map(|app| if app.reduce_motion_on { 1 } else { 0 })
        .unwrap_or(0)
}

#[no_mangle]
pub extern "C" fn oxideui_host_emit_pinch(cx: f32, cy: f32, delta: f32) {
    let _ = with_app_mut(|app| {
        if let Some(router) = app.router.as_mut() {
...
#[no_mangle]
pub extern "C" fn oxideui_host_emit_double_tap() {
    let _ = with_app_mut(|app| {
        if let Some(router) = app.router.as_mut() {
            router.input_double_tap();
        }
...
#[cfg(target_os = "ios")]
extern "C" {
    fn oxideui_host_ios_log(ptr: *const ::libc::c_char, len: usize);
}
...
fn ios_log(msg: &str) {
    ...
    unsafe {
        if ios_log_enabled() {
            oxideui_host_ios_log(msg.as_ptr() as *const ::libc::c_char, msg.len());
        }
    }
}
