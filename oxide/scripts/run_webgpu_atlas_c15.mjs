...
const samples = process.argv[3] || "1";
const frames = process.argv[4] || "24";
const prefix = process.argv[5] || "cold";
const output = process.argv[6] || "";
if (!root) {
   throw new Error("usage: run_webgpu_atlas_c15.mjs WEB_ROOT [SAMPLES] [FRAMES] [PREFIX] [OUTPUT]");
}
...
try {
   ...
   const result = await app.bench_webgpu_atlas_c15(Number(params.get("samples")), Number(params.get("frames")));
   await fetch("/__c15_result", { method: "POST", body: result });
} catch (error) {
   await fetch("/__c15_result", { method: "POST", body: "error=" + (error?.stack || error) });
}
</script></body></html>`;
...
const server = createServer((request, response) => {
   if (request.method === "POST" && request.url === "/__c15_result") {
      let body = "";
   ...
   if (request.url.split("?", 1)[0] === "/c15-atlas-bench.html") {
   ...
   const relative = request.url.split("?", 1)[0] === "/" ? "/c15-atlas-bench.html" : request.url.split("?", 1)[0];
...
const chromeArgs = [
   ...
   "--noerrdialogs",
   "--use-angle=metal",
   `http://127.0.0.1:${port}/c15-atlas-bench.html?samples=${samples}&frames=${frames}`,
];
const child = chromeArch
   ? spawn("arch", [`-${chromeArch}`, chrome, ...chromeArgs])
   : spawn(chrome, chromeArgs);
child.stderr.resume();
const timeout = setTimeout(() => rejectResult(new Error("C15 browser benchmark timed out")), 120000);
try {
