...
pub fn generate_checker_rgba(width: u32, height: u32) -> Vec<u8>
{
   ...
   for y in 0..height {
      for x in 0..width {
         let idx = ((y as usize).saturating_mul(width as usize).saturating_add(x as usize)).saturating_mul(4);
...
mod wasm_host
{
   ...
   const DEFAULT_FONT_BYTES: &[u8] = include_bytes!("../../../../crates/ui-core/assets/Asap-Regular.ttf");
   ...
   impl AppState
   {
      fn frame_at(&mut self, timestamp_ms: f64) -> Result<(), JsValue>
      {
         ...
         self.renderer
            .borrow_mut()
            .resize(physical_w, physical_h, scale)
            .map_err(render_err)?;
         ...
         self.builder.clear();
         let viewport = gfx::RectF::new(0.0, 0.0, physical_w as f32 / scale.max(1.0), physical_h as f32 / scale.max(1.0));
   ...
   impl OxideWebApp
   {
      ...
      pub async fn new_async(canvas_id: &str) -> Result<OxideWebApp, JsValue>
      {
         ...
         let renderer = BrowserRenderer::from_canvas_id_webgpu(canvas_id).await.map_err(render_err)?;
      ...
      fn new_with_renderer(mut renderer: BrowserRenderer) -> Result<OxideWebApp, JsValue>
      {
         ...
         let _font_id = router.text.fonts.add_font(text::Font::from_bytes(DEFAULT_FONT_BYTES.to_vec()));

         let (image_w, image_h) = (256_u32, 256_u32);
         let checker = generate_checker_rgba(image_w, image_h);
         let image = renderer
            .borrow_mut()
            .image_create_rgba8(image_w, image_h, &checker, (image_w as usize).saturating_mul(4));
      ...
      pub fn bench_frames(&self, frames: u32) -> Result<String, JsValue>
      {
         ...
         for frame in 0..frame_count {
            self.state
               .borrow_mut()
               .frame_at(start + frame as f64 * 16.666_667)?;
         }
      ...
      pub fn bench_frame_samples(&self, samples: u32, frames_per_sample: u32) -> Result<String, JsValue>
      {
   ...
   async fn webgpu_call_promise(receiver: &JsValue, method: &'static str) -> Result<JsValue, JsValue>
   {
      let method_value = js_sys::Reflect::get(receiver, &JsValue::from_str(method))?;
      let method_fn = method_value
         .dyn_into::<js_sys::Function>()
         .map_err(|_| JsValue::from_str(method))?;
   ...
   fn install_event_listeners(state: &Rc<RefCell<AppState>>) -> Result<(), JsValue>
   {
      ...
      install_resize_listener(state, window_target)?;
      install_redraw_listener(state, window_target)?;
   ...
   fn create_ime_textarea(canvas: &HtmlCanvasElement) -> Result<HtmlTextAreaElement, JsValue>
   {
      ...
      let document_body = document.body().ok_or_else(|| JsValue::from_str("document body is unavailable"))?;
      document_body.append_child(textarea.unchecked_ref())?;
      canvas.set_attribute("tabindex", "0")?;
      Ok(textarea)
   }

   fn install_pointer_listener(state: &Rc<RefCell<AppState>>, target: &EventTarget, name: &'static str) -> Result<(), JsValue>
   {
      ...
      let closure = Closure::wrap(Box::new(move |event: Event| {
         ...
         let (x, y) = event_point(&state_for_event.borrow().canvas, pointer.client_x() as f32, pointer.client_y() as f32);
   ...
   fn install_wheel_listener(state: &Rc<RefCell<AppState>>, target: &EventTarget) -> Result<(), JsValue>
   {
      let state_for_event = Rc::clone(state);
      let closure = Closure::wrap(Box::new(move |event: Event| {
         ...
         event.prevent_default();
         let (x, y) = event_point(&state_for_event.borrow().canvas, wheel.client_x() as f32, wheel.client_y() as f32);
   ...
   fn install_keyboard_listener(state: &Rc<RefCell<AppState>>, target: &EventTarget, name: &'static str) -> Result<(), JsValue>
   {
      let state_for_event = Rc::clone(state);
      let closure = Closure::wrap(Box::new(move |event: Event| {
   ...
   fn install_ime_listeners(state: &Rc<RefCell<AppState>>, window_target: &EventTarget) -> Result<(), JsValue>
   {
      ...
      target.add_event_listener_with_callback("compositionstart", start.as_ref().unchecked_ref())?;
      state.borrow_mut().listeners.push(start);
      ...
      target.add_event_listener_with_callback("compositionupdate", update.as_ref().unchecked_ref())?;
      state.borrow_mut().listeners.push(update);
      ...
      target.add_event_listener_with_callback("compositionend", end.as_ref().unchecked_ref())?;
      state.borrow_mut().listeners.push(end);
      ...
      state.borrow_mut().listeners.push(input);
      ...
      let show = Closure::wrap(Box::new(move |_event: Event| {
         ...
         state.router.input_set_ime_rect(gfx::RectF::new(0.0, logical_h * 0.62, 0.0, logical_h * 0.38));
      }) as Box<dyn FnMut(Event)>);
      window_target.add_event_listener_with_callback("oxide-ime-show", show.as_ref().unchecked_ref())?;
      state.borrow_mut().listeners.push(show);
      ...
      window_target.add_event_listener_with_callback("oxide-ime-hide", hide.as_ref().unchecked_ref())?;
      state.borrow_mut().listeners.push(hide);
      Ok(())
   }

   fn install_resize_listener(state: &Rc<RefCell<AppState>>, target: &EventTarget) -> Result<(), JsValue>
   {
      let state_for_event = Rc::clone(state);
   ...
   fn install_redraw_listener(state: &Rc<RefCell<AppState>>, target: &EventTarget) -> Result<(), JsValue>
   {
      let state_for_event = Rc::clone(state);
      let closure = Closure::wrap(Box::new(move |_event: Event| {
         let _ = state_for_event.borrow_mut().frame_at(perf_now());
      }) as Box<dyn FnMut(Event)>);
      target.add_event_listener_with_callback("oxide-redraw", closure.as_ref().unchecked_ref())?;
      state.borrow_mut().listeners.push(closure);
      Ok(())
   }

   fn route_key(router: &mut scenes::Router<WebUploader>, event: &KeyboardEvent, down: bool, ime_focused: bool) -> bool
   {
      let key = event.key();
      if ime_focused {
         if down {
...
#[cfg(target_arch = "wasm32")]
pub use wasm_host::{platform_smoke_report, start_oxide, start_oxide_async, webgpu_smoke_report, OxideWebApp};
