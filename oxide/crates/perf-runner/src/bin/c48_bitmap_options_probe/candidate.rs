...
use oxide_ui_core::bitmap_text::BitmapTextAtlas;
use oxide_ui_core::{
...
struct Report
{
   ...
   atlas_upload_calls_per_op: u64,
   atlas_upload_bytes_per_op: u64,
   allocs_per_op: f64,
...
fn main()
{
   ...
   atlas.set_handle(ImageHandle(1));
   ...
   {
      encoder.clear();
      black_box(draw_text_input_options_popover(
         &mut encoder,
         &mut atlas,
         2.0,
         layout,
   ...
   for _ in 0..SAMPLE_COUNT
   {
      ...
      {
         encoder.clear();
         let ready = draw_text_input_options_popover(
            &mut encoder,
            &mut atlas,
            2.0,
            layout,
         ...
         checksum = checksum
            .wrapping_add(black_box(encoder.commands()))
            .wrapping_add(u64::from(black_box(ready)));
      }
   ...
   let report = Report {
      variant: "candidate-atlas-glyph-run",
      warmup_samples_us,
      samples_us,
      operations_per_sample: OPS_PER_SAMPLE,
      commands_per_op,
      solid_draws_per_op,
      label_solid_draws_per_op: 0,
      glyph_runs_per_op,
      render_mutex_locks_per_op: 0,
      atlas_upload_calls_per_op: 0,
      atlas_upload_bytes_per_op: 0,
...
fn run_metal()
{
   ...
   let (data, width, height) = atlas.image();
   metal_probe::measure_metal(
      "candidate-atlas-glyph-run",
      encoder.into_inner(),
      Some(metal_probe::AtlasImage { data, width, height }),
      popovers,
   );
}
