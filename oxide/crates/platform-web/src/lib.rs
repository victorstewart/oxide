...
) -> Result<BrowserMediaStream, api::PlatformError>
{
   ...
   let stream = JsFuture::from(promise)
      ...
      .map_err(|_| api::PlatformError::Unknown(String::from("getUserMedia did not return MediaStream")))?;
...
impl api::Bluetooth for WebBluetooth
{
   ...
   fn read(&self, _id: api::PeripheralId, _chr: api::GattChar) -> Result<Vec<u8>, api::PlatformError>
   {
   ...
   fn notify(&self, _id: api::PeripheralId, _chr: api::GattChar, _enable: bool) -> Result<(), api::PlatformError>
   {
...
{
   fn save<'a>(&'a self, key: &'a str, data: &'a [u8]) -> Pin<Box<dyn Future<Output = Result<(), api::PlatformError>> + Send + 'a>>
   {
   ...
   fn delete<'a>(&'a self, key: &'a str) -> Pin<Box<dyn Future<Output = Result<(), api::PlatformError>> + Send + 'a>>
   {
...
impl api::media_library::MediaLibrary for WebMediaLibrary
{
   fn query_assets(
      ...
      _limit: u32,
      _offset: u32,
   ) -> Pin<Box<dyn Future<Output = Result<Vec<api::media_library::MediaAsset>, api::PlatformError>> + Send + '_>>
   {
   ...
   ) -> Pin<Box<dyn Future<Output = Result<api::media_library::AssetData, api::PlatformError>> + Send + '_>>
   {
      Box::pin(async { Err(unsupported("media library images are not supported by this backend")) })
   }
   ...
   {
      Box::pin(async { Err(unsupported("media library videos are not supported by this backend")) })
   }
}
...
fn refresh_rate_probe_sample(timestamp_ms: f64)
{
   ...
   if done {
      let hz = REFRESH_SAMPLES.with(|samples| refresh_rate_hz_from_frame_deltas(&samples.borrow()));
...
{
   let callbacks = PERMISSION_SUBSCRIBERS.with(|subscribers| {
      std::mem::take(&mut *subscribers.borrow_mut())
   });
...
fn location_request_once_inner() -> Result<(), api::PlatformError>
{
   ...
   LOCATION_ONESHOT.with(|slot| {
      *slot.borrow_mut() = Some(LocationOneShot {
         id,
         _success: success,
         _error: error,
      });
...
fn location_position_options(opts: api::LocationOptions) -> web_sys::PositionOptions
{
   ...
   position_options.set_enable_high_accuracy(
      opts.precise || opts.accuracy == api::LocationAccuracy::Precise,
   );
   ...
   LOCATION_STATE.with(|state| {
      ...
      if state.history.len() >= LOCATION_HISTORY_LIMIT {
         let remove = state
            ...
            .len()
            .saturating_add(1)
            .saturating_sub(LOCATION_HISTORY_LIMIT);
...
{
   let callbacks = LOCATION_SUBSCRIBERS.with(|subscribers| {
      std::mem::take(&mut *subscribers.borrow_mut())
   });
   for callback in callbacks.iter() {
...
fn network_notify()
{
   ...
   let callbacks = NETWORK_SUBSCRIBERS.with(|subscribers| {
      std::mem::take(&mut *subscribers.borrow_mut())
   });
   for callback in callbacks.iter() {
      callback(status);
...
) -> Result<u32, api::PlatformError>
{
   ...
   let iframe = document
      ...
      .map_err(|_| api::PlatformError::Unknown(String::from("created element was not an iframe")))?;
   ...
   let error = Closure::wrap(Box::new(move |_event: web_sys::Event| {
      web_view_emit(id, api::web_view::WebViewEvent::LoadFailed(unsupported("iframe load failed")));
   }) as Box<dyn FnMut(web_sys::Event)>);
...
{
   let callback = WEB_VIEWS.with(|views| {
      views
         .borrow_mut()
         .get_mut(&id)
         .and_then(|entry| entry.on_event.take())
   });
...
#[cfg(target_arch = "wasm32")]
fn web_view_same_origin_url(window: &web_sys::Window, url: &str) -> Result<String, api::PlatformError>
{
   if url == "about:blank" {
   ...
   if parsed.origin() != origin {
      return Err(api::PlatformError::PermissionDenied("browser iframe web views require same-origin URLs"));
   }
   Ok(parsed.href())
}
...
fn wake_lock_request()
{
   ...
   let Ok(wake_lock) = js_sys::Reflect::get(navigator.as_ref(), &JsValue::from_str("wakeLock")) else {
      return;
   };
