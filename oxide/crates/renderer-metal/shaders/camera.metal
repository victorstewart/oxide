...
vertex CamVSOut v_inst_rect_cam(uint vid [[vertex_id]],
...
fragment float4 f_camera_nv12(CamVSOut in [[stage_in]],
  ...
  if (xy.x < p.rect.x || xy.y < p.rect.y || xy.x > (p.rect.x + p.rect.z) ||
      xy.y > (p.rect.y + p.rect.w)) {
    discard_fragment();
  }
...
fragment float4 f_camera_nv12_preview_fast_full(
    CamVSOut in [[stage_in]], texture2d<half> yTex [[texture(0)]],
    texture2d<half> uvTex [[texture(1)]], sampler s [[sampler(0)]],
    constant CamParams *parr [[buffer(1)]]) {
  CamParams p = parr[in.iid];
  float2 d = (in.pos_dp - p.rect.xy) / max(p.rect.zw, float2(1e-5));
  float2 uv_norm = d * p.uv_scale + p.uv_bias;
...
fragment float4 f_camera_nv12_preview_fast_video(
  ...
  float2 d = (in.pos_dp - p.rect.xy) / max(p.rect.zw, float2(1e-5));
  float2 uv_norm = d * p.uv_scale + p.uv_bias;
  half y_sample = yTex.sample(s, uv_norm).r;
  half2 uv_sample = uvTex.sample(s, uv_norm).rg - half2(0.5h, 0.5h);
  constexpr half y_min = half(16.0 / 255.0);
  constexpr half y_scale = half(255.0 / 219.0);
  constexpr half chroma_scale = half(255.0 / 224.0);
  half y = clamp((y_sample - y_min) * y_scale, 0.0h, 1.0h);
  half u = uv_sample.x * chroma_scale;
  half v = uv_sample.y * chroma_scale;
  half3 rgb =
      half3(y + 1.5748h * v, y - 0.1873h * u - 0.4681h * v, y + 1.8556h * u);
  half g = half(clamp(p.grayscale, 0.0, 1.0));
  half3 base = mix(rgb, half3(y, y, y), g);
...
fragment float4 f_camera_bgra_bench(CamVSOut in [[stage_in]],
  ...
  float2 d = (xy - p.rect.xy) / max(p.rect.zw, float2(1e-5));
  float2 uv_norm = d * p.uv_scale + p.uv_bias;
  float4 sample_rgba = rgbTex.sample(s, uv_norm);
  float luma = dot(sample_rgba.rgb, float3(0.2126, 0.7152, 0.0722));
