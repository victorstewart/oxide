...
fn direct_alloc_and_dealloc_update_counters() {
    ...
    assert_eq!(after_dealloc.dealloc_count.saturating_sub(after_alloc.dealloc_count), 1,);
    assert_eq!(after_dealloc.dealloc_bytes.saturating_sub(after_alloc.dealloc_bytes), 64,);
}
...
fn direct_realloc_updates_realloc_and_live_byte_deltas() {
    ...
    assert_eq!(after_grow.realloc_grow_bytes.saturating_sub(before.realloc_grow_bytes), 64,);
    ...
    assert_eq!(after_shrink.realloc_count.saturating_sub(after_grow.realloc_count), 1,);
    assert_eq!(
        after_shrink.realloc_shrink_bytes.saturating_sub(after_grow.realloc_shrink_bytes),
        48,
    );
