...
use oxide_platform_api::{
   ...
   Bluetooth, BluetoothEvent, CameraConfig, CameraDevice, CameraFrame, CameraImage,
   CameraManager, CameraRecording, CameraStream, CaptureMode, ColorSpace, Connection,
   ConnectionEvent, ConnectionGroup, ConnectionOptions, FlashMode, GeoHash, GeoRegion,
   GeoRegionTracker, GattChar, HttpClient, HttpMethod, HttpRequest, HttpResponse,
   LocationAccuracy, LocationEvent, LocationOptions, LocationReading, LocationService, MotionSample,
   MotionService, NetworkError, NetworkErrorDomain, Networking, PeripheralId, PeripheralInfo,
   PhotoEvent, PhotoOptions, PlatformError, ProtocolOptions, PushManager, PushNotification,
   PushPresentation, PushProvider, PushToken, RecordingContainer, RecordingDestination,
   RecordingEvent, RecordingOptions, RecordingResult, RestorationInfo, ScanOptions, SecureStorage,
   TcpOptions, TorchMode, UdpEvent, UdpPacket, UdpSocket as OxideUdpSocket,
};
...
extern "C" {
   ...
   fn oxide_host_set_push_token_callback(
...
fn peripheral_info_from_raw(raw: &AppleBleScanInfo) -> PeripheralInfo
{
   ...
   {
      let slice = unsafe {
         core::slice::from_raw_parts(raw.services_ptr, raw.service_count * 16)
      };
   ...
   {
      let bytes = unsafe { core::slice::from_raw_parts(raw.manufacturer_ptr, raw.manufacturer_len) };
      Some(bytes.to_vec())
   }
   else
   {
...
extern "C" {
   ...
   fn oxide_cam_start_native_preview_layer() -> i32;
   fn oxide_cam_stop();
...
struct AppleNativePreviewStream {
    active: AtomicBool,
}
...
impl CameraStream for AppleNativePreviewStream {
    fn stop(&self) {
...
impl Drop for AppleNativePreviewStream {
    fn drop(&mut self) {
        self.stop();
    }
}
...
impl AppleCameraManager {
    ...
    fn start_native_preview_capture(&self) -> Result<(), PlatformError> {
        ...
        if rc != 0 {
            return Err(camera_error_from_rc(rc, "native camera preview start failed"));
        }
        unsafe {
            let _ = oxide_host_set_camera_running(1);
        }
...
fn remove_camera_subscriber(id: u64) {
    ...
    if should_stop {
        if CAM_STATE.native_previews.load(Ordering::SeqCst) > 0 {
            APPLE_CAMERA_MANAGER.stop_capture();
            let _ = APPLE_CAMERA_MANAGER.start_native_preview_capture();
        } else {
            APPLE_CAMERA_MANAGER.stop_capture();
        }
    }
...
impl CameraManager for AppleCameraManager {
    ...
    fn start_native_preview(
        ...
        Ok(Box::new(AppleNativePreviewStream { active: AtomicBool::new(true) }))
    }
    ...
    fn select_device(&self, device: CameraDevice) {
        let mut settings = CAM_STATE.settings.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
...
{
   fn new(stream: TcpStream, on_event: alloc::boxed::Box<dyn Fn(ConnectionEvent) + Send>) -> Result<Self, PlatformError>
   {
...
impl AppleUdpSocket
{
   fn new(socket: UdpSocket, on_event: alloc::boxed::Box<dyn Fn(UdpEvent) + Send>) -> Result<Self, PlatformError>
   {
      socket
...
impl Networking for AppleSocketNetworking
{
   ...
   ) -> Result<alloc::boxed::Box<dyn Connection + Send>, PlatformError>
   {
      let tcp = match options.protocol
      {
         ...
         ProtocolOptions::Quic(_) => return Err(PlatformError::Invalid("connect_tcp requires TCP options")),
      };
      validate_tcp_options(&options, tcp)?;
      let stream = TcpStream::connect((options.host.as_str(), options.port)).map_err(platform_io_error)?;
...
fn validate_tcp_options(options: &ConnectionOptions, tcp: &TcpOptions) -> Result<(), PlatformError>
{
   ...
   {
      return Err(PlatformError::Unsupported("raw TCP TLS is not implemented for the Apple socket backend"));
   }
   if tcp.fast_open
   {
      return Err(PlatformError::Unsupported("TCP Fast Open is not implemented for the Apple socket backend"));
   }
   Ok(())
}
...
fn configure_tcp_stream(stream: &TcpStream, tcp: &TcpOptions) -> Result<(), PlatformError>
{
   ...
   if tcp.keepalive
   {
      return Err(PlatformError::Unsupported("TCP keepalive configuration requires an Apple socket backend"));
   }
   Ok(())
}

#[cfg(any(target_os = "macos", target_os = "ios"))]
fn set_socket_int(fd: libc::c_int, level: libc::c_int, name: libc::c_int, value: libc::c_int) -> Result<(), PlatformError>
{
   let rc = unsafe {
...
fn bind_udp_socket(local_port: u16) -> Result<UdpSocket, PlatformError>
{
   ...
   Err(platform_io_error(last_err.unwrap_or_else(|| std::io::Error::from(IoErrorKind::AddrNotAvailable))))
}
...
fn network_error_from_io(err: &std::io::Error) -> NetworkError
{
   let domain = match err.kind()
   {
      ...
      IoErrorKind::ConnectionRefused | IoErrorKind::ConnectionReset | IoErrorKind::ConnectionAborted =>
         NetworkErrorDomain::Posix,
...
type LocationCallback =
   Arc<Mutex<alloc::boxed::Box<dyn Fn(LocationEvent) + Send + 'static>>>;
type MotionCallback =
   Arc<Mutex<alloc::boxed::Box<dyn Fn(MotionSample) + Send + 'static>>>;

static APPLE_LOCATION_INIT: Once = Once::new();
static APPLE_LOCATION_SUBS: Lazy<Mutex<alloc::vec::Vec<LocationCallback>>> =
   Lazy::new(|| Mutex::new(alloc::vec::Vec::new()));
static APPLE_LOCATION_LAST: Lazy<Mutex<Option<LocationReading>>> =
...
fn update_region_events(reading: LocationReading) -> alloc::vec::Vec<LocationEvent>
{
   ...
   let mut state = APPLE_LOCATION_REGIONS
      .lock()
      .unwrap_or_else(std::sync::PoisonError::into_inner);
...
pub unsafe extern "C" fn oxide_location_update_trampoline(sample: *const AppleLocationSample)
{
   ...
   {
      let mut hist = APPLE_LOCATION_HISTORY
         .lock()
         .unwrap_or_else(std::sync::PoisonError::into_inner);
...
impl LocationService for AppleLocationService
{
   ...
   {
      let rc = unsafe { oxide_host_location_set_accuracy(location_accuracy_to_apple_kind(accuracy)) };
      if rc != 0
      {
...
pub unsafe extern "C" fn oxide_motion_trampoline(sample: *const AppleMotionSample)
{
   ...
   {
      let mut hist = APPLE_MOTION_HISTORY
         .lock()
         .unwrap_or_else(std::sync::PoisonError::into_inner);
...
static APPLE_PUSH_TOKEN: Lazy<Mutex<Option<PushToken>>> =
   Lazy::new(|| Mutex::new(None));
...
impl PushManager for ApplePushManager
{
   ...
   {
      if let Some(token) = APPLE_PUSH_TOKEN
         .lock()
         .unwrap_or_else(std::sync::PoisonError::into_inner)
         .clone()
      {
...
impl oxide_platform_api::web_view::WebView for AppleWebView
{
   ...
   >
   {
      ...
      else
      {
         ...
         {
            Err(PlatformError::Unknown(alloc::string::String::from("web view host returned null script result")))
         }
         else
         {
            let bytes = unsafe { core::slice::from_raw_parts(ptr, len) };
...
impl AppleMediaLibraryManager
{
      ...
      F: FnOnce(*const u8, usize, &mut AppleMediaImageData) -> i32,
   {
      ...
      let bytes =
         unsafe { core::slice::from_raw_parts(image_data.data_ptr, image_data.data_len).to_vec() };
      unsafe {
...
impl MediaLibrary for AppleMediaLibraryManager
{
      ...
      alloc::boxed::Box::pin(async move {
         ...
         for idx in 0..count
         {
            ...
            assets.push(MediaAsset {
               id: AssetId(copy_string(raw.identifier_ptr, raw.identifier_len).unwrap_or_default()),
   ...
   fn request_image_data(
      ...
      alloc::boxed::Box<dyn core::future::Future<Output = Result<AssetData, PlatformError>> + Send + '_>,
   >
   {
...
#[must_use]
pub fn reachability_state_from_apple_path(status: u32, iface: u32, expensive: bool) -> ReachabilityState
{
   if status == 0
   {
...
#[must_use]
pub fn network_interface_from_apple_path_kind(iface: u32) -> oxide_platform_api::network_status::NetworkInterface
{
   match iface
   {
...
#[must_use]
pub fn network_status_from_apple_path(status: u32, iface: u32) -> oxide_platform_api::network_status::NetworkStatus
{
...
#[must_use]
pub fn permission_domain_from_apple_code(code: u32) -> Option<oxide_platform_api::PermissionDomain>
{
   match code
   {
...
impl AppleSecureStorage
{
   ...
   pub fn save_sync(&self, key: &str, data: &[u8]) -> Result<(), PlatformError>
   {
      ...
      {
         0 => Ok(()),
         code => Err(PlatformError::Unknown(alloc::format!("secure storage save failed: {code}"))),
      }
   }
   ...
   pub fn load_sync(&self, key: &str) -> Result<Option<alloc::vec::Vec<u8>>, PlatformError>
   {
      ...
      match result
      {
         ...
         1 => Ok(None),
         code => Err(PlatformError::Unknown(alloc::format!("secure storage load failed: {code}"))),
      }
   }
   ...
   pub fn delete_sync(&self, key: &str) -> Result<(), PlatformError>
   {
      ...
      {
         0 | 1 => Ok(()),
         code => Err(PlatformError::Unknown(alloc::format!("secure storage delete failed: {code}"))),
      }
   }
}
