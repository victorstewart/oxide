use oxide_platform_apple::{
   ...
   AppleCamPhotoEvent, AppleCamRecordEvent, AppleCameraManager, AppleHttpClient, AppleHttpResponse,
   AppleLocationConfig, AppleLocationSample,
   AppleLocationService, AppleMediaAsset, AppleMediaImageData, AppleMediaLibraryManager,
   AppleMotionSample, AppleMotionService, ApplePushManager, AppleSecureStorage,
...
use oxide_platform_apple::{
   ...
   oxide_push_token_trampoline, permission_domain_from_apple_code, permission_domain_to_apple_code,
   permission_status_from_apple_code, permission_status_to_apple_code, reachability_state_from_apple_path,
   APPLE_INTERFACE_CELLULAR, APPLE_INTERFACE_WIFI, APPLE_INTERFACE_WIRED,
   APPLE_PATH_KIND_CELLULAR, APPLE_PATH_KIND_OTHER, APPLE_PATH_KIND_WIFI, APPLE_PATH_KIND_WIRED,
   APPLE_PERMISSION_AUTHORIZED, APPLE_PERMISSION_CAMERA, APPLE_PERMISSION_DENIED,
   APPLE_PERMISSION_NOT_DETERMINED,
};
...
use oxide_platform_api::media_library::{AssetData, AssetId, AssetType, ImageQuality, MediaLibrary};
#[cfg(feature = "web-view-macos")]
...
static PUSH_REGISTER_CALLS: std::sync::atomic::AtomicUsize =
...
static PUSH_CLEAR_DELIVERED_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
static BLE_INIT_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
static BLE_SCAN_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
static BLE_STOP_SCAN_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
static BLE_CONNECT_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
static BLE_DISCONNECT_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
static BLE_WRITE_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
static BLE_NOTIFY_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
...
static CAMERA_START_PREVIEW_ONLY_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
static CAMERA_START_NATIVE_PREVIEW_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
static CAMERA_STOP_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
static CAMERA_RECORD_START_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
static CAMERA_PHOTO_CALLS: std::sync::atomic::AtomicUsize =
   std::sync::atomic::AtomicUsize::new(0);
static CAMERA_START_DEFAULT_RC: AtomicI32 = AtomicI32::new(0);
static CAMERA_START_PREVIEW_ONLY_RC: AtomicI32 = AtomicI32::new(0);
static CAMERA_START_NATIVE_PREVIEW_RC: AtomicI32 = AtomicI32::new(0);
...
#[cfg(feature = "web-view-macos")]
static WEB_VIEW_LAST_ID: std::sync::atomic::AtomicU64 =
   std::sync::atomic::AtomicU64::new(0);

fn push_token_callback_cell(
) -> &'static Mutex<Option<unsafe extern "C" fn(u32, *const u8, usize)>>
{
...
fn camera_frame_callback_cell(
) -> &'static Mutex<Option<unsafe extern "C" fn(*const AppleCamFrame)>>
{
...
fn camera_audio_callback_cell(
) -> &'static Mutex<Option<unsafe extern "C" fn(*const AppleCamAudio)>>
{
...
pub extern "C" fn oxide_host_http_response_free(response: *mut AppleHttpResponse)
{
   ...
   unsafe {
      ...
      {
         drop(Vec::from_raw_parts(response.final_url_ptr, response.final_url_len, response.final_url_len));
      }
      if !response.content_type_ptr.is_null()
      {
         drop(Vec::from_raw_parts(response.content_type_ptr, response.content_type_len, response.content_type_len));
      }
...
#[no_mangle]
pub extern "C" fn oxide_cam_start_native_preview_layer() -> i32
{
   CAMERA_START_NATIVE_PREVIEW_CALLS.fetch_add(1, Ordering::SeqCst);
   CAMERA_START_NATIVE_PREVIEW_RC.load(Ordering::SeqCst)
}
...
fn reset_camera_return_codes()
{
   ...
   CAMERA_START_NATIVE_PREVIEW_RC.store(0, Ordering::SeqCst);
...
{
   if NATIVE_MOTION_RUNNING.load(Ordering::SeqCst) { 1 } else { 0 }
}
...
#[cfg(feature = "web-view-macos")]
#[no_mangle]
pub extern "C" fn oxide_web_view_create(
   url_ptr: *const u8,
   url_len: usize,
   id: u64,
) -> i32
...
fn fill_media_image(out_image: *mut AppleMediaImageData, bytes: &[u8], width: u32, height: u32) -> i32
{
...
fn apple_media_library_loads_image_and_video_data()
{
   ...
   let video = block_on_ready(
      library.request_video_data(&AssetId(String::from("video-a"))),
   )
...
fn apple_media_library_maps_host_return_codes_to_platform_errors()
{
   ...
   assert!(matches!(io_image, PlatformError::Io(message) if message == "media image rgba request failed"));

   let missing_video = block_on_ready(
      library.request_video_data(&AssetId(String::from("missing-video"))),
   )
   .expect_err("missing video should fail");
   assert!(matches!(missing_video, PlatformError::NotFound("media video file")));

   let invalid_video = block_on_ready(
      library.request_video_data(&AssetId(String::from("invalid-video"))),
   )
   ...
   let unsupported_video = block_on_ready(
      library.request_video_data(&AssetId(String::from("unsupported-video"))),
   )
...
fn apple_push_manager_registers_caches_token_and_fans_out_notifications()
{
   ...
   unsafe {
      notify_cb(br#"{"message":"hello","count":3}"#.as_ptr(), br#"{"message":"hello","count":3}"#.len());
   }
...
fn assert_apple_socket_tcp_echo(tcp: TcpOptions)
{
   ...
   loop
   {
      let has_pong = events.lock().expect("TCP events").iter().any(|event| {
         matches!(event, ConnectionEvent::Read(bytes) if bytes == b"pong")
      });
      if has_pong
      {
         break;
...
fn apple_socket_networking_binds_udp_sends_and_reads()
{
   ...
   loop
   {
      let has_pong = events.lock().expect("UDP events").iter().any(|event| {
         matches!(event, UdpEvent::Read(packet) if packet.data == b"pong")
      });
      if has_pong
      {
         break;
...
fn apple_bluetooth_forwards_controls_and_read_write_notify()
{
   ...
   assert!(events.lock().expect("BLE events").iter().any(|event| {
      matches!(event, BluetoothEvent::StateChanged { powered_on: true })
   }));
   ...
   bluetooth.start_scan(&ScanOptions {
   ...
   assert_eq!(
      bluetooth.read(id, chr.clone()).expect("BLE read"),
      b"ble-read"
   );
   bluetooth
      .write(id, chr.clone(), b"ble-write", true)
      .expect("BLE write");
...
fn apple_bluetooth_emits_discovery_cache_and_notifications()
{
   ...
   assert!(events.iter().any(|event| {
      matches!(event, BluetoothEvent::Connected(event_id) if *event_id == id)
   }));
   assert!(events.iter().any(|event| {
      match event
      {
...
fn apple_camera_manager_maps_host_return_codes_to_platform_errors()
{
   ...
   CAMERA_START_NATIVE_PREVIEW_RC.store(-1, Ordering::SeqCst);
   let err = match camera.start_native_preview(oxide_platform_api::CameraConfig::default()) {
      Ok(stream) => {
         stream.stop();
         panic!("native camera preview unexpectedly started");
      }
      Err(err) => err,
   };
   assert!(matches!(err, PlatformError::Unsupported("native camera preview start failed")));
   reset_camera_return_codes();

   CAMERA_RECORD_START_RC.store(-3, Ordering::SeqCst);
   let err = match camera.start_recording(
      oxide_platform_api::RecordingOptions::default(),
      Box::new(|_| {}),
   ) {
      Ok(recording) => {
...
fn apple_web_view_service_creates_executes_emits_and_closes()
{
   ...
   let sink = events.clone();
   let view = service
      .create_view("https://example.invalid", Box::new(move |event| {
   ...
   assert_ne!(id, 0);
   assert!(web_view_callback_cell()
      .lock()
      .expect("web view callback cell")
      .is_some());
   ...
   let undefined = block_on_ready(view.execute_script("undefined")).expect("undefined script result");
   ...
   assert!(matches!(failed, PlatformError::Unknown(message) if message == "web view script failed"));
   let copy_failed = block_on_ready(view.execute_script("copy-fail")).expect_err("copy failed script");
   assert!(matches!(copy_failed, PlatformError::Io(message) if message == "web view result copy failed"));
...
{
   assert_eq!(reachability_state_from_apple_path(0, APPLE_PATH_KIND_WIFI, false), ReachabilityState::Offline);

   let wifi = reachability_state_from_apple_path(1, APPLE_PATH_KIND_WIFI, false);
   assert!(matches!(wifi, ReachabilityState::Online { path } if path.kind == NetworkPathKind::Wifi && !path.expensive));

   let cellular = reachability_state_from_apple_path(1, APPLE_PATH_KIND_CELLULAR, true);
   assert!(matches!(cellular, ReachabilityState::Online { path } if path.kind == NetworkPathKind::Cellular && path.expensive));

   let wired = reachability_state_from_apple_path(1, APPLE_PATH_KIND_WIRED, false);
   assert!(matches!(wired, ReachabilityState::Online { path } if path.kind == NetworkPathKind::Wired));

   let other = reachability_state_from_apple_path(1, APPLE_PATH_KIND_OTHER, false);
   assert!(matches!(other, ReachabilityState::Online { path } if path.kind == NetworkPathKind::Other));
}
...
fn apple_permission_codes_round_trip_known_values()
{
   ...
   assert_eq!(permission_domain_from_apple_code(APPLE_PERMISSION_CAMERA), Some(PermissionDomain::Camera));
   ...
   assert_eq!(permission_status_from_apple_code(APPLE_PERMISSION_AUTHORIZED), PermissionStatus::Authorized);
