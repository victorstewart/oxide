...
mod tests {
    ...
    use super::{
        compare_visual_tree_snapshots, VisualTreeNode, VisualTreeSnapshot, VisualTreeViewport,
    };

    fn test_snapshot(root: VisualTreeNode) -> VisualTreeSnapshot {
        VisualTreeSnapshot {
            schema: "oxide.visual_tree.v1".to_owned(),
            ...
            viewport: VisualTreeViewport {
                frame: super::VisualTreeRect { x: 0.0, y: 0.0, w: 100.0, h: 100.0 },
                safe: super::VisualTreeInsets { left: 0.0, top: 0.0, right: 0.0, bottom: 0.0 },
                points_scale: 1.0,
            },
            root,
        }
    ...
    fn test_node() -> VisualTreeNode {
        VisualTreeNode::new("root/card", "BigFaceNode", RectF { x: 0.0, y: 0.0, w: 10.0, h: 10.0 })
    }

    #[test]
    fn visual_tree_data_compare_accepts_equivalent_numeric_encodings() {
        let reference = test_snapshot(
            test_node()
                .with_data("tunnel_alpha", json!(1))
                .with_data("tunnel_scale", json!(0.9706518054008483)),
        );
        let candidate = test_snapshot(
            test_node()
                .with_data("tunnel_alpha", json!(1.0))
                .with_data("tunnel_scale", json!(0.9706518054008484)),
        );
    ...
    fn visual_tree_data_compare_rejects_meaningful_numeric_deltas() {
        let reference = test_snapshot(test_node().with_data("tunnel_alpha", json!(0.5)));
        let candidate = test_snapshot(test_node().with_data("tunnel_alpha", json!(0.51)));
