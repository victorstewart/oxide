...
impl UrlComponents {
    ...
    pub fn parse(url: &str) -> Option<Self> {
        // Support both authority-based schemes (`://`) and colon-only schemes (`mailto:`, `tel:`, etc.)
        let (scheme, rest, has_authority) = if let Some(idx) = url.find("://") {
            (url[..idx].to_string(), &url[idx + 3..], true)
        } else if let Some(idx) = url.find(':') {
            (url[..idx].to_string(), &url[idx + 1..], false)
        } else {
            return None;
        };

        // Split host/path/query
        let (authority_or_path, query_str) = if let Some(q_idx) = rest.find('?') {
        ...
        let (host, path) = if has_authority {
            if let Some(slash_idx) = authority_or_path.find('/') {
                (
                    Some(authority_or_path[..slash_idx].to_string()),
                    Some(authority_or_path[slash_idx..].to_string()),
                )
            } else if !authority_or_path.is_empty() {
                (Some(authority_or_path.to_string()), None)
            } else {
                (None, None)
            }
        } else {
            let trimmed = authority_or_path.trim();
...
impl UrlSchemeSecurity {
    ...
    pub fn is_allowed(&self, url: &str) -> Result<(), String> {
        let components =
            UrlComponents::parse(url).ok_or_else(|| String::from("Invalid URL format"))?;
        ...
        if !self.allowed_schemes.is_empty()
            && !self.allowed_schemes.contains(&components.scheme)
            && !self.allow_custom
        {
