...
impl Atlas {
    ...
    fn grow_or_reset(&mut self) {
        ...
        if self.width < MAX_SIZE {
            new_w = (self.width * 2).min(MAX_SIZE);
        } else if self.height < MAX_SIZE {
            new_h = (self.height * 2).min(MAX_SIZE);
        } else {
        ...
        for (info, pos) in infos.iter().zip(poss.iter()) {
            ...
            let entry = if let Some(e) = atlas.map.get_mut(&key) {
                ...
                let (ax, ay) = match atlas.alloc_rect(aw, ah) {
                    Some(rc) => rc,
                    None => {
                        // Avoid mid-run atlas reset/growth because it invalidates UVs of glyphs
                        // already emitted in this draw list.
                        pen_x += pos.x_advance as f32 / 64.0;
                        pen_y += pos.y_advance as f32 / 64.0;
                        continue;
                    }
            ...
            if run_vertex_base.saturating_add(4) > u16::MAX as u32 {
                // Glyph indices are encoded as u16; truncate this run safely.
                break;
            }
            ...
            push_v(draw_vertices, gx + gw, gy + gh, u1, v1, rgba); // 3
                                                                   // Indices are local to this glyph run's vertex span.
...
mod tests {
    ...
    #[test]
    fn push_i_preserves_local_index_values() {
        let mut indices = Vec::new();
        push_i(&mut indices, 12, 13, 14);
        assert_eq!(indices, vec![12, 13, 14]);
    }
}
