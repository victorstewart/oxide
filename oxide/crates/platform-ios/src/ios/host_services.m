...
static uint32_t oxide_status_from_av_authorization(AVAuthorizationStatus status)
{
   switch (status)
   {
...
static int32_t nametag_status_from_av_authorization(AVAuthorizationStatus status)
{
   switch (status)
   {
   case AVAuthorizationStatusAuthorized:
...
static uint32_t oxide_status_from_photo_authorization(
...
static int32_t nametag_status_from_photo_authorization(
    PHAuthorizationStatus status)
{
   switch (status)
   {
...
static uint32_t oxide_status_from_contact_authorization(
...
static uint32_t oxide_status_from_notification_settings(
...
static int32_t nametag_location_permission_status(void)
{
   ...
   CLAuthorizationStatus status = ensure_location_permission_manager().authorizationStatus;
   switch (status)
   {
   case kCLAuthorizationStatusAuthorizedAlways:
   case kCLAuthorizationStatusAuthorizedWhenInUse:
...
static UIImpactFeedbackGenerator *impact_generator(UIImpactFeedbackStyle style)
{
   switch (style)
   {
...
void oxide_host_haptics_play(uint32_t pattern)
{
   dispatch_main_async(^{
     ...
     case 3:
        ...
        break;
     case 5:
        [notification_generator() notificationOccurred:UINotificationFeedbackTypeWarning];
        [notification_generator() prepare];
        break;
     case 6:
        [notification_generator() notificationOccurred:UINotificationFeedbackTypeError];
        [notification_generator() prepare];
        break;
     case 4:
     default:
        [notification_generator() notificationOccurred:UINotificationFeedbackTypeSuccess];
        [notification_generator() prepare];
        break;
     }
   });
