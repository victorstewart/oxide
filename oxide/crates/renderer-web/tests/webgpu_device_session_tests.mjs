...
test("separate wasm modules reuse one page-session device across route transitions", async () => {
   ...
   assert.notStrictEqual(recoveredAdapter, landingAdapter);
   assert.notStrictEqual(recoveredDevice, landingDevice);
   assert.equal(nativeAdapterRequests, 132);
   ...
   assert.deepEqual(
      ...
      { requests: 2, live: 0, leases: 0, destroys: 1, shutdowns: 1, closed: true },
   );
   assert.throws(
