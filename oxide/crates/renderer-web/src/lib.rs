...
#[cfg_attr(not(target_arch = "wasm32"), allow(dead_code))]
fn normalized_index_mode(source: &[u16], vertex_base: u32, vertex_count: u32) -> Option<NormalizedIndexMode>
{
   if source.is_empty() {
...
mod wasm
{
   ...
   use super::{
      a8_to_rgba, color_cache_key, color_to_css, copy_a8_rows, copy_rgba_rows, logical_dimension,
      layer_physical_dimension, normalized_index_mode, resolve_index, sanitize_scale, WebRendererStats,
   };
   use oxide_renderer_api as api;
   ...
   impl WebRenderer
   {
      ...
      fn encode_draw_cmd(&mut self, list: &api::DrawList, item: &api::DrawCmd)
      {
         match item {
            ...
            api::DrawCmd::Solid { vb, ib, color } => self.draw_solid_span(list, *vb, *ib, *color),
            api::DrawCmd::Image { tex, dst, src, alpha } => self.draw_image_rect(*tex, *dst, *src, *alpha),
            api::DrawCmd::GlyphRun { run } => self.draw_glyph_run_from_list(list, run),
            api::DrawCmd::RRect { rect, radii, color } => self.draw_rrect_path(*rect, *radii, *color),
            api::DrawCmd::NineSlice { tex, rect, slice, alpha } => self.draw_nine_slice(*tex, *rect, *slice, *alpha),
            api::DrawCmd::Backdrop { rect, sigma, tint, alpha } => self.draw_backdrop_fallback(*rect, *sigma, *tint, *alpha),
            api::DrawCmd::VisualEffect { rect, effect } => {
               let tint = effect.tint();
               self.draw_backdrop_fallback(*rect, effect.blur_intensity() * 72.0, tint, tint.a);
            }
            ...
            api::DrawCmd::Spinner { center, atom, alpha } => self.draw_spinner_shape(*center, *atom, *alpha),
            api::DrawCmd::ClipPush { rect } => self.push_clip(*rect),
            api::DrawCmd::ClipPop => self.pop_clip(),
         }
      }

      fn encode_layer(&mut self, list: &api::DrawList, index: &mut usize, id: u32, rect: api::RectF, dirty: bool)
      {
      ...
      fn cached_layer_canvas(&self, id: u32, width: u32, height: u32) -> Option<HtmlCanvasElement>
      {
         if id == 0 {
            return None;
         }
         let layer = self.layers.get(&id)?;
         if layer.width == width && layer.height == height && (layer.scale - self.scale).abs() <= f32::EPSILON {
      ...
      ) -> Result<(), api::RenderError>
      {
         ...
         layer_ctx
            .set_transform(scale, 0.0, 0.0, scale, -(rect.x as f64) * scale, -(rect.y as f64) * scale)
      ...
      fn draw_solid_span(&mut self, list: &api::DrawList, vb: api::VertexSpan, ib: api::IndexSpan, color: api::Color)
      {
         ...
         if ib.len > 0 {
            ...
            for tri in indices.chunks_exact(3) {
               let Some(a) = resolve_index(tri[0], mode).and_then(|idx| vertices.get(idx)) else {
                  continue;
               };
               let Some(b) = resolve_index(tri[1], mode).and_then(|idx| vertices.get(idx)) else {
                  continue;
               };
               let Some(c) = resolve_index(tri[2], mode).and_then(|idx| vertices.get(idx)) else {
                  continue;
               };
      ...
      fn draw_image_rect(&mut self, handle: api::ImageHandle, dst: api::RectF, src: api::RectF, alpha: f32)
      {
      ...
      )
      {
         ...
         let result = self.ctx.draw_image_with_html_canvas_element_and_sw_and_sh_and_dx_and_dy_and_dw_and_dh(
            &image.canvas,
            sx,
            sy,
            sw,
      ...
      fn draw_glyph_run_vertices(&mut self, run: &api::GlyphRun, vertices: &[api::Vertex], indices: &[u16])
      {
         if !indices.is_empty() {
      ...
      fn tinted_or_base_canvas(&mut self, handle: api::ImageHandle, color: api::Color) -> Option<HtmlCanvasElement>
      {
         ...
         match &mut image.kind {
            ...
            WebImageKind::A8 { tinted, .. } => {
               ...
               let canvas = tinted_canvas(&image.canvas, image.width, image.height, color).ok()?;
      ...
      fn draw_nine_slice(&mut self, handle: api::ImageHandle, rect: api::RectF, slice: api::Insets, alpha: f32)
      {
         let Some(image) = self.image(handle) else {
            return;
         };
      ...
      fn draw_backdrop_fallback(&mut self, rect: api::RectF, sigma: f32, tint: api::Color, alpha: f32)
      {
         if rect.w <= 0.0 || rect.h <= 0.0 {
            return;
         }
         if self.draw_sampled_backdrop(rect, sigma).is_none() {
            self.draw_tint_rect(rect, api::Color::rgba(tint.r, tint.g, tint.b, tint.a * alpha.clamp(0.0, 1.0)));
            return;
         }
         self.draw_tint_rect(rect, api::Color::rgba(tint.r, tint.g, tint.b, tint.a * alpha.clamp(0.0, 1.0)));
      }
      ...
      fn draw_spinner_shape(&mut self, center: [f32; 2], atom: f32, alpha: f32)
      {
         ...
         for idx in 0..12 {
            ...
            self.ctx.begin_path();
            self.ctx.set_fill_style_str(&css);
            let _ = self.ctx.arc(x as f64, y as f64, (atom * 0.22).max(1.0) as f64, 0.0, core::f64::consts::TAU);
            self.ctx.fill();
            self.stats.draws = self.stats.draws.saturating_add(1);
         }
      }
   ...
   impl api::Renderer for WebRenderer
   {
      ...
      fn resize(&mut self, width: u32, height: u32, scale: f32) -> Result<(), api::RenderError>
      {
         ...
         style.set_property("width", &css_w).map_err(|err| js_error("canvas style width", err))?;
         style.set_property("height", &css_h).map_err(|err| js_error("canvas style height", err))?;
         let s = scale as f64;
         self.ctx.set_transform(s, 0.0, 0.0, s, 0.0, 0.0).map_err(|err| js_error("canvas transform", err))?;
         Ok(())
      }
   }
   ...
   impl api::RenderEncoder for WebRenderer
   {
      ...
      fn draw_nine_slice(&mut self, img: api::ImageHandle, rect: api::RectF, slice: api::Insets, alpha: f32)
      {
      ...
      fn draw_glyph_run_resolved(&mut self, run: &api::GlyphRun, vertices: &[api::Vertex], indices: &[u16])
      {
         self.draw_glyph_run_vertices(run, vertices, indices);
      }
   }
   ...
   fn canvas_context(canvas: &HtmlCanvasElement) -> Result<CanvasRenderingContext2d, api::RenderError>
   {
      let value = canvas
         .get_context("2d")
      ...
      value
         ...
         .map_err(|_| api::RenderError::InvalidOperation("context is not CanvasRenderingContext2d"))
   }
   ...
   #[allow(dead_code)]
   fn canvas_from_rgba(width: u32, height: u32, rgba: &[u8]) -> Result<HtmlCanvasElement, api::RenderError>
   {
      let canvas = create_canvas(width, height)?;
      let ctx = canvas_context(&canvas)?;
      let image_data = ImageData::new_with_u8_clamped_array_and_sh(Clamped(rgba), width, height)
   ...
   {
      (
         layer_physical_dimension(rect.w, scale),
         layer_physical_dimension(rect.h, scale),
      )
   }
