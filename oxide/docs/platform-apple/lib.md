...
- Camera startup/control failures preserve host return-code categories for permission denied, missing device, busy, invalid, I/O, and unsupported cases. Recording/photo callback failures preserve their event status categories, and native preview leases keep capture alive without requiring app-visible frame subscribers.
