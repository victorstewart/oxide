...
Device perf runs reuse the same case mapping but add process-scoped Instruments attachment. The harness first launches the app through CoreDevice, waits for its foreground-ready marker and exact PID, attaches Metal System Trace to that process, and starts the workload only after Instruments confirms tracing. This avoids Instruments' unreliable app-launch path while preserving exact environment, process, and workload ownership. CPU metrics still come from XCTest, while external GPU timing, GPU counters, and the canonical device phase/signpost timings come from process-scoped Metal System Trace on the same case. Every XCTest cadence payload carries its exact test name, so the UIKit path first reuses the matching cadence summary from the batched metrics pass when trace console output is unavailable. A separate console-summary pass remains a fail-closed fallback only when neither that exact-test payload nor attached-launch stdout contains cadence; the harness never substitutes another test's cadence, compositor timing, or inferred zeros. Required camera and static-idle console capture runs once before the trace retry loop, so counter or handshake retries cannot repeat that workload. Both UIKit and Oxide allow one bounded retry when the physical-device ready/start/attach handshake flakes, and they share one command-local GPU-counter capability: an explicit rejection, startup timeout, or wall-time timeout suppresses the optional profile for later cases in that command. Oxide on-screen rows also merge host-console stage summaries so in-app Metal command-buffer and timestamp-counter timings survive even when Apple rejects or times out the optional Instruments hardware-counter profile. Energy remains an optional imported input sourced from manual Power Profiler traces, and the report marks it as manual-pending when those traces are absent.
...
- 2026-08-09: applied the existing one-retry device trace handshake policy symmetrically to on-screen Oxide traces.
...
- 2026-08-08: cached a timed-out optional GPU-counter profile for the rest of the device command, avoiding identical per-case probes while preserving direct GPU timing and Metal System Trace.
- 2026-08-07: shared explicit GPU-counter capability across each device command while keeping transient timeouts case-local.
- 2026-08-07: keyed XCTest frame-cadence payloads by exact test name and reused those batched summaries before launching a separate cadence fallback pass.
