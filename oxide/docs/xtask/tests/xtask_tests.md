...
- `experiment_manifest_checker_accepts_current_manifest()`: validates every committed experiment entry and freezes the current total plus accepted/rejected decision counts, including the accepted C34 Metal ID-mask field packing and C33/C32 field-cache decisions.
