...
C38 likewise moves ordinary WebGPU images to one 36-byte destination/UV/alpha instance over a renderer-owned static indexed quad. Adjacent commands share a draw only while image, format, target, and clip remain identical, so texture order is unchanged. Image meshes retain generic geometry; direct and prepared reports expose image instance, triangle, and byte totals.

C39 moves nine-slices to one 44-byte instance over a renderer-owned fixed cell grid. C40 moves spinners to one 20-byte instance whose twelve atoms are generated in WGSL; the shared viewport/property uniform carries animation phase, transform, and opacity. C41 moves each neon marker from three RRect records to one 60-byte analytic instance and matches the corrected Metal core/ring/Gaussian semantics. `WebRendererStats` exposes spinner and neon-marker instance, triangle, and byte totals.

C46 moves WebGPU glyphs to one 36-byte destination/UV/RGBA8 instance over a shared four-corner triangle strip. Compatible consecutive ranges batch by atlas page, A8/SDF pipeline, target, and clip. Dynamic streams and prepared chunk buffers/bundles share the same representation, while malformed or non-quad input retains the generic fallback. The opt-in browser matrix reproduces Metal C45's 1,000-label Latin/RTL/CJK/emoji, four-page bitmap/SDF workload without expanding the default committed report.
