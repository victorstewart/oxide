...
- `wasm_webgpu_device_session_is_js_realm_owned_page_scoped_and_observable()`: freezes the cross-WASM JavaScript lifecycle coordinator, compatible page-owned adapter/device reuse, module-lifetime wgpu registry/handle roots, route-local lease ownership, device-loss recovery, terminal pagehide closure, and stable read-only counters.
...
`webgpu_device_session_tests.mjs` is a categorical lifecycle regression: two separately evaluated module copies and 128 alternating renderer replacements reuse one compatible page-owned adapter/device generation, incompatible adapter/device requirements fail closed, device loss creates a fresh generation, route release preserves the live generation, and terminal shutdown destroys it exactly once. The Rust source contract additionally keeps one wgpu instance and the latest adapter/device/queue handles rooted per compiled WASM module so renderer replacement cannot retire external handles another module still uses. No frame path is exercised or changed.
