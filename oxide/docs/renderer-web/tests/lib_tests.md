...
`webgpu_device_session_tests.mjs` is a categorical lifecycle regression: two separately evaluated module copies and 128 alternating route leases each receive a device owned by their creating wgpu instance, release destroys each completed renderer generation, and terminal shutdown destroys every still-live generation exactly once. The Rust source contract additionally requires the originating `wgpu::Adapter` and `wgpu::Instance` to remain owned after dependent resources and before the JavaScript session lease so destruction follows resource, adapter, instance, lease order. No frame path is exercised or changed.
