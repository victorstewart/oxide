...
- `wasm_webgpu_device_session_is_js_realm_owned_page_scoped_and_observable()`: freezes the cross-WASM JavaScript coordinator, unchanged Rust constructors, renderer-owned adapter/instance lifetime and teardown order, route-local lease ownership, compatible request reuse, terminal pagehide destroy, and stable read-only counters.
...
- 2026-07-23: froze renderer-owned wgpu adapter/instance lifetime and dependent-resource teardown order.
