...
- `wasm_webgpu_device_session_is_js_realm_owned_page_scoped_and_observable()`: freezes the JavaScript lifecycle coordinator, compatible page-owned adapter/device reuse, the module-retained wgpu instance and renderer-owned clones/adapter wrappers, route-local lease ownership, device-loss recovery, terminal pagehide closure, and stable read-only counters.
...
- `webgpu_glyphs_use_compact_ordered_instances_in_dynamic_and_prepared_paths()`: freezes the 36-byte RGBA8 instance layout, four-corner WGSL expansion, accepted quad topologies, ordered page/pipeline batching, and dynamic plus prepared-buffer/bundle wiring.
...
`webgpu_device_session_tests.mjs` is a categorical lifecycle regression: two renderer leases and 128 route transitions in one module reuse one compatible page-owned device, a separately evaluated helper copy fails closed, incompatible adapter/device requirements fail closed, route-local Rust device destruction is suppressed, eight consecutive device-request failures retry without a false live generation, device loss creates a fresh generation, route release preserves it, and terminal shutdown invokes the original destroy exactly once. The Rust source contract additionally requires a module-retained originating `wgpu::Instance` and each renderer's `wgpu::Adapter` and `wgpu::Instance` clone to remain owned after dependent resources and before the JavaScript session lease. No frame path is exercised or changed.
...
- 2026-07-23: froze renderer-owned wgpu adapter and instance lifetimes and dependent-resource teardown order.
- 2026-07-22: froze the JavaScript-realm device lifecycle protocol, observable counters, cross-module instance isolation, bounded route teardown, and terminal pagehide destruction.
