...
- `wasm_webgpu_device_session_is_js_realm_owned_page_scoped_and_observable()`: freezes the JavaScript lifecycle coordinator, first-submission serialization through the GPU completion fence, the module-retained wgpu instance and renderer-owned clones/adapter wrappers, failure/release recovery, terminal pagehide closure, and stable read-only counters.
...
`webgpu_device_session_tests.mjs` is a categorical lifecycle regression: two renderer leases initialize strictly in order, both failed-release and explicit-success completion unblock the next owner, a separately evaluated helper copy fails closed, persisted pagehide preserves the session, and terminal pagehide releases queued initialization exactly once before closing future acquisition. The Rust source contract additionally requires a module-retained originating `wgpu::Instance`, each renderer's `wgpu::Adapter` and `wgpu::Instance` clone, and one-shot completion only after the first submitted-work GPU fence. No steady-state frame path is changed.
