...
`webgpu_device_session_tests.mjs` is a categorical lifecycle regression: two separately evaluated module copies and 128 alternating renderer replacements each acquire a distinct adapter while reusing one compatible page-owned device, incompatible device requirements fail closed, device loss creates a fresh generation, route release preserves the live generation, and terminal shutdown destroys it exactly once. The Rust source contract additionally requires each originating `wgpu::Adapter` and `wgpu::Instance` wrapper to remain owned after dependent resources and before the JavaScript session lease. No frame path is exercised or changed.
