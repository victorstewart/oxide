...
- `wasm_webgpu_device_session_is_js_realm_owned_page_scoped_and_observable()`: freezes the cross-WASM JavaScript lifecycle coordinator, serialized cold adapter/device initialization, renderer-owned instance/adapter/device ownership, direct queue-completion fence, route-local lease ownership, terminal pagehide closure, and stable read-only counters without intercepting browser WebGPU objects.
...
`webgpu_device_session_tests.mjs` is a categorical lifecycle regression: two separately evaluated module copies serialize initialization while retaining distinct native adapter/device owners, then 32 alternating adapter/device failures release their exact lease and permit the queued renderer to recover with a fresh owner. Terminal page shutdown closes acquisition without intercepting or destroying wgpu-owned browser objects. No frame path is exercised or changed.
