...
- `wasm_webgpu_device_session_is_js_realm_owned_page_scoped_and_observable()`: freezes the cross-WASM JavaScript coordinator, renderer-owned adapter and instance lifetimes and teardown order, route-local lease ownership, compatible request reuse, terminal pagehide destroy, and stable read-only counters.
