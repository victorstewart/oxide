...
- `MetalRenderer` constructs the four ID-mask pipeline states and calls this module from `encode_id_mask_gpu_compositor`.
...
- `MetalRenderer::readback_id_mask_snapshot(&self) -> Option<IdMaskSnapshotReadback>` with `snapshot-tests`
  - Reads exact R8 IDs and the final RGBA32F city/seam fields for verification only.
