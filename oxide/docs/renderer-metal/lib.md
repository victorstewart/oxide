...
Ordinary Oxide fragments emit straight RGB plus coverage alpha. Their Metal pipelines therefore use source alpha for RGB, one for source alpha, and one-minus-source-alpha for both destination factors. This keeps source-over composition from multiplying source alpha into itself and preserves an opaque destination's alpha. Retained layer composites remain the separate premultiplied source-over case because the cached layer texture already contains composited RGB.
...
- Invalid RGBA dimensions, stride, source coverage, or checked layout arithmetic return the invalid zero handle without creating a Metal texture, advancing an image ID, or changing upload/residency statistics. A zero `row_bytes` value remains the documented tight-row request.
...
- 2026-08-07: corrected straight-alpha source-over factors across flat and prepared UI pipelines while retaining premultiplied cached-layer composition.
