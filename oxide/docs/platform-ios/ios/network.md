...
- Ticket/resumption, TLS 1.3-only TCP/TLS, public-API-only early-data handling, QUIC option application, keepalive, path/cache unpinning, receive permit accounting, queue budgets, and monotonic overall send deadlines are guarded by `network_bridge_tests.rs` source-level assertions.
