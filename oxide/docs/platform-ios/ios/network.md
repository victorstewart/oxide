...
- Every configured trust anchor must contain exactly one DER certificate. Hostname suppression changes only leaf-name matching and never permits an untrusted, non-server, expired, or otherwise invalid chain.
...
- Custom trust policy and anchor evaluation occur once per TLS handshake and add no steady-state frame or packet-path work.
