...
| `oxide-timing` | `crates/timing` | lib | monotonic clock, timer wheel, global animation manager (`anim` module) | Unit tests for eases, timers, and compatibility-reserved motion switching | add fake-clock harness for long-running animation scenarios and multi-threaded timer stress |
