...
- `webgpu_browser_capture_script_compares_pixels_against_golden()`: verifies the browser recapture script still compares pixels, supports app, Scene3D, and ID-mask capture targets, retries transient blank/mismatched visual captures before report writes, and can write JSON/Markdown WebGPU baseline reports with pacing, pass-family, timestamp-attribution, duplicate benchmark-report Chrome trace, per-benchmark User Timing marks, resource-lifetime, warm-resource-churn, upload-scratch, effect-uniform direct GPU timestamp totals, backdrop-batch, Scene3D, draw-state, clip-state, and pixel-check fields.
- `committed_webgpu_browser_baseline_persists_nonzero_id_mask_ab_rows()`: parses `benchmarks/web/latest.json` and verifies the 24-row browser WebGPU matrix is present with nonzero timing, frame-pacing fields, pass-family counters, Chrome trace event counts and benchmark User Timing labels from the duplicate benchmark-report run, zero WASM memory growth across benchmark marks after prewarm, zero warm-frame sampler creation, a report-level current-row warm-resource-churn zero-growth summary, upload-scratch A/B, effect-uniform A/B with direct GPU timestamp totals, backdrop-batch A/B, the Scene3D stress rows, draw-state cache A/B, clip-state cache A/B, and current-path wins.

## Logic narrative

The first test checks RGBA buffer shape. The second test samples different tile positions and confirms they differ, which catches accidental one-color placeholder output. The static shell test catches regressions where the HTML page points at the wrong wasm-bindgen output path, stops invoking the backend smoke and perf hooks, stops probing timestamp-query capability, stops marking benchmark families with browser User Timing, stops publishing the hidden report JSON, stops honoring capture-target query parameters, stops waiting after ID-mask capture, stops using the no-RAF deterministic app snapshot path for app captures, or stops logging the browser-test markers. The source-inspection tests keep the browser-only WebGPU A/B exports, explicit app/Scene3D/ID-mask snapshot render hooks, direct-capture guard, bounded visual-capture retry, effect-uniform GPU timestamp fields, backdrop-batch, clip-state, Chrome trace, benchmark mark, zero WASM memory-growth, and warm-resource-churn report contracts, and recapture script contract visible to native CI without launching Chrome. The committed-golden tests decode browser PNGs so missing files, wrong dimensions, blank captures, and app-vs-compositor target mixups fail in native tests. The persisted-report test prevents committed browser baselines from silently dropping the A/B rows, frame-pacing fields, pass-family counters, timestamp-attribution status, duplicate benchmark-report Chrome trace event counts and benchmark labels, zero WASM memory growth after prewarm, resource-lifetime counters, effect-uniform counters and direct GPU timestamp totals, backdrop-batch counters, clip-depth/scissor counters, the current-row warm-resource-churn zero-growth summary, capture target, or from regressing back to virtual-time zero measurements.
