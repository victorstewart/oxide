...
C38 source coverage freezes the dedicated 100/1,000 same/mixed-texture rounded-image matrix, its zero/uniform/asymmetric radius pattern, instance/triangle/byte/coalescing/pass metrics, isolated page mode, and `image` capture route. The runtime artifacts additionally prove exact DPR and prepared-transform pixels.
...
- 2026-07-18: required the C38 architecture adapter to exercise rounded-image radii while retaining its compatible batching and counter contract.
...
- 2026-07-17: aligned the committed browser-report assertion with intentionally promoted report version 6.
