...
- `host_exposes_webgpu_id_mask_ab_benchmark()`: verifies the wasm host keeps the current-vs-legacy WebGPU ID-mask, upload, upload-scratch, effect-uniform, backdrop-batch, Scene3D, mixed, layer/effects, command-family, draw-state, and clip-state benchmark exports, exports p50/p95/p99/peak/avg plus missed-frame/hitch fields, exposes the explicit app and ID-mask browser snapshot render hooks, and keeps the direct-capture guard that prevents resize/redraw events from repainting the app over the ID-mask capture.
