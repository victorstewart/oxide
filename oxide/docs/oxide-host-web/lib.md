...
- `oxide_host_web::OxideWebApp::bench_webgpu_upload_ab(&self, samples: u32, frames_per_sample: u32) -> Result<String, JsValue>`: runs WebGPU glyph-atlas A8 and RGBA image upload samples comparing dirty-subrect updates against full-texture updates while drawing equivalent output.
...
- `oxide_host_web::OxideWebApp::bench_webgpu_mixed_matrix(&self, samples: u32, frames_per_sample: u32) -> Result<String, JsValue>`: runs a warm mixed command-encoding workload with image, glyph, rounded-rect, clip, layer marker, backdrop, visual effect, spinner, damage, and present work.
- `oxide_host_web::OxideWebApp::bench_webgpu_layer_effects_matrix(&self, samples: u32, frames_per_sample: u32) -> Result<String, JsValue>`: runs a dedicated warm WebGPU layer/damage/effects workload with nested layer markers, multiple damage rectangles, image/glyph content, backdrop sampling, visual effect, spinner, texture-copy, pass-family, timestamp, and resource-churn counters.
- `oxide_host_web::OxideWebApp::bench_webgpu_command_family_matrix(&self, samples: u32, frames_per_sample: u32) -> Result<String, JsValue>`: runs a dedicated warm WebGPU generic draw-command workload covering `ImageMesh`, `NineSlice`, and SDF glyphs while asserting zero web `CameraBg` work without adding product-specific renderer hooks.
