...
The browser host is single-threaded and uses `Rc<RefCell<_>>` rather than cross-thread synchronization. The draw-list builder is reused between frames to preserve allocation capacity. C16 geometry diagnostics build their three large draw lists once and reuse them for every measured frame so renderer lowering, not authoring allocation, owns the measured delta. Event listener closures and the hidden IME textarea intentionally live for the app lifetime.
