...
- canvas backing-size selection
- `oxide_renderer_web::BrowserRenderer::from_canvas_webgpu`
...
- `oxide_host_web::OxideWebApp::render_webgpu_prepared_snapshot(&self, flat_control: bool) -> Result<String, JsValue>`: renders the deterministic 256-chunk C25 scene through either exact flattening or persistent prepared replay for pixel comparison and RAF pacing.
...
- `oxide_host_web::OxideWebApp::bench_webgpu_prepared_chunks(&self, samples: u32, frames_per_sample: u32, dirty: bool, flat_control: bool, bundle_threshold: u32) -> Promise<String>`: measures clean or one-dirty C25 encode, command, queue-wait, completed-frame, direct-GPU, cache, upload, bundle, direct-draw, and residency distributions.
- `oxide_host_web::OxideWebApp::bench_webgpu_dynamic_properties(&self, samples, frames_per_sample) -> Promise<String>`: measures the C26 300-instance text/image workload with backend CPU, command encoding, queue wait, direct GPU, event-to-submit, geometry/property traffic, cache, and ring counters.
- `oxide_host_web::OxideWebApp::render_webgpu_dynamic_property_snapshot(&self, phase, full_affine, flat_control)`: replays cached translation/opacity or full-affine snapshots for RAF pacing and visual checks.
- `oxide_host_web::OxideWebApp::bench_webgpu_prepared_guardrails(&self) -> Promise<String>`: exercises structural revision, hybrid clipping, tiny scenes, dynamic instances, effects, layers, device generation, resize, resource update/release/recreation, and zero-budget behavior in real Chrome.
...
- `oxide_host_web::OxideWebApp::bench_webgpu_geometry_c16(&self, samples: u32, frames_per_sample: u32) -> Result<String, JsValue>`: runs non-default 10,000-glyph-quad, 10,000-image-quad, and 70,002-vertex large-mesh samples over immutable prebuilt draw lists, reporting CPU, GPU, allocation, copied-byte, and upload-byte evidence.
- `oxide_host_web::OxideWebApp::bench_webgpu_targets_c19(&self, repeats: u32) -> Promise<String>`: runs non-default direct, selectively prewarmed backdrop, and selectively prewarmed Scene3D first-use sequences across alternating physical sizes, reporting construction/resize/prewarm resources, logical target bytes, CPU-ready/completion timing, and direct GPU timestamps.
...
The non-default C25 page mode builds a deterministic 256-chunk snapshot once, alternates one chunk's geometry revision only when the dirty workload requests it, and reuses the same image/atlas resources. The flat control explicitly flattens into caller-owned storage; the candidate calls the renderer's prepared snapshot entry point. The dedicated RAF adapter renders exactly once per real callback after 64 warmups, while the lifecycle adapter verifies cache and invalidation counters outside the timed population. The prepared capture target feeds both paths through the existing PNG comparator rather than coupling screenshots to timing processes.
...
The browser host is single-threaded and uses `Rc<RefCell<_>>` rather than cross-thread synchronization. The draw-list builder is reused between frames to preserve allocation capacity. C16 geometry diagnostics build their three large draw lists once and reuse them for every measured frame so renderer lowering, not authoring allocation, owns the measured delta. C19 reuses one renderer while alternating dimensions and releases its two Scene3D meshes after the bounded run; every fresh-process pair constructs a new browser app. C25 allocates its snapshots and sample vectors before measured inner loops, then relies on renderer-owned persistent prepared storage; each paired side still starts in a fresh Chrome process. Event listener closures and the hidden IME textarea intentionally live for the app lifetime.
