...
The browser host is single-threaded and uses `Rc<RefCell<_>>` rather than cross-thread synchronization. The draw-list builder is reused between frames to preserve allocation capacity. C16 geometry diagnostics build their three large draw lists once and reuse them for every measured frame so renderer lowering, not authoring allocation, owns the measured delta. C19 reuses one renderer while alternating dimensions and releases its two Scene3D meshes after the bounded run; every fresh-process pair constructs a new browser app. Event listener closures and the hidden IME textarea intentionally live for the app lifetime.
