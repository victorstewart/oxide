...
- `web_latest_report_satisfies_webgpu_distribution_and_pacing_contract`
  - Test-only report gate that validates `benchmarks/web/latest.json` keeps the full 24-row browser WebGPU matrix populated with p50/p95/p99/peak, missed-frame, hitch, resource-lifetime counters, draw-family counters, draw-state and clip-state counters, image-upload temp/scratch counters, effect-uniform write/byte/slot counters plus direct GPU timestamp A/B totals, texture-copy/render-pass counters, pass-family counters, timestamp-query attribution status, Chrome browser trace summary fields and per-benchmark User Timing labels from a duplicate benchmark-report run, upload summaries, upload-scratch summaries, effect-uniform summaries, backdrop-batch summaries, Scene3D summaries, mixed-command coverage, dedicated layer/damage/effects coverage, generic command-family coverage, draw-state cache A/B coverage, clip-state cache A/B coverage, a warm-resource-churn summary proving current warm rows have zero post-warmup growth, zero WASM memory growth across benchmark marks after prewarm, and pixel-check fields.
...
The resulting `PerfReport` is serialized to JSON and Markdown. Baseline comparison is median-based and only applies to gated cases. Coverage validation is structural: the suite is considered incomplete if any required family lacks a case. The separate browser WebGPU baseline under `benchmarks/web/latest.json` is guarded by a report-shape test so the 24-row browser matrix cannot drop latency distributions, 60 Hz/120 Hz missed-frame and hitch metrics, upload summaries, upload-scratch summaries, effect-uniform summaries with direct GPU timestamp A/B totals, backdrop-batch summaries, Scene3D summaries, mixed-command coverage, dedicated layer/damage/effects coverage, generic command-family coverage, draw-state cache A/B coverage, clip-state cache A/B coverage, the warm-resource-churn zero-growth summary for current warm rows, zero WASM memory growth across benchmark marks after prewarm, Chrome browser trace event counts and benchmark User Timing labels from a duplicate benchmark-report run, resource lifetime counters, image-upload temp/scratch counters, effect-uniform counters, texture-copy/render-pass counters, pass-family counters, timestamp-query attribution status, or pixel checks while still looking like a valid persisted browser report.
...
- Invariants maintained:
  ...
  - Browser WebGPU report rows cannot pass the report tests without p50/p95/p99/peak, missed-frame, hitch, pixel-check, draw-family counters, draw-state counters, clip-depth/scissor counters, pass-family counters that sum to total render passes, resource-lifetime counters, image-upload temp/scratch counters, effect-uniform counters plus current and legacy direct GPU timestamp totals for the effect-uniform A/B row, texture-copy counters, upload A/B fields, upload-scratch A/B fields, effect-uniform A/B fields, backdrop-batch A/B fields, Scene3D A/B fields, mixed-command coverage, dedicated layer/damage/effects coverage, generic command-family coverage, draw-state cache A/B fields, clip-state cache A/B fields, the report-level warm-resource-churn zero-growth summary, zero WASM memory growth across benchmark marks after prewarm, Chrome browser trace event counts with `capture_phase=benchmark-report` and `timing_source=untraced-baseline-report`, traced browser User Timing labels for every benchmark family, timestamp-query attribution status, and collected timestamp pass counts that match render-pass counts when timestamp queries are supported.
