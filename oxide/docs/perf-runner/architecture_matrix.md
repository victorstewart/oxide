...
- Animation/text: a real 300-node `UiSurface` driven by `Animator`, retained glyph/image replay, nested opacity/clips/transforms, hit testing, and accessibility dirtiness; warm/new/script/fallback/atlas/scale text cases, actual immutable variable-font construction, pinned Latin-width/CJK-weight 3x physical A8 raster plus warm cache proof, a 200-new-label production Metal publication row, and the atlas-backed text-option popover path. Apple glyph-instance rows require the native bitmap backend; non-Apple rows additionally require the SDF path. The paged-atlas locality stimulus uses native-size glyphs that deterministically evict one older page while preserving the recently referenced unrelated page.
...
- Metal analytic instances: physical Metal rows for RRect, square image, rounded image, nine-slice, spinner, backdrop, and visual-effect ordered runs at 1/64/1,024/10,000 instances. Rounded-image rows use the same texture, source, destination, alpha, and cardinalities as square-image rows, changing only the four 3-point corner radii. Every row persists frame/encode/direct-GPU distributions, draws, instances, upload bytes, analytic ring bytes/binds/growth, total growth, and frame-ring residency.
...
- Variable-font report tests require the public authoring row to construct one two-axis immutable instance and the 3x architecture row to record pinned axes, cold misses/rasterizations, and an immediate warm replay with cache hits but zero misses, rasterizations, or dirty pixels.
...
## Changelog
- 2026-07-26: made the variable-axis matrix instantiate pinned coordinates and exercise physical-resolution A8 output.
- 2026-07-18: added public variable-font construction and pinned Latin/CJK 3x cold-shape/raster plus warm-cache evidence rows.
- 2026-07-18: added 1/64/1,024/10,000 rounded-image analytic instance rows and a radius-payload workload assertion.
